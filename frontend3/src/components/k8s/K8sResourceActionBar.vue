<script setup lang="ts">
  ...
  const missingPermission = (action: ActionItem): boolean =>
    Boolean(action.permission) && !authStore.hasPermission(action.permission);

  const isDisabled = (action: ActionItem): boolean => Boolean(action.disabled) || missingPermission(action);

  const actionTooltip = (action: ActionItem): string =>
    missingPermission(action) ? `缺少权限：${action.permission}\n请联系管理员在角色管理中开通` : action.tooltip || '';
