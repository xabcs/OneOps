...
import { flattenMenuTree, cleanMenuTree, filterMenuTree, findSiblings } from './modules/menu-tree-helper';
...
async function handleMove(row: Api.SystemManage.Menu, direction: 'up' | 'down') {
  ...
  try {
    await Promise.all([fetchUpdateMenu(row.id, { sort: targetRow.sort }), fetchUpdateMenu(targetRow.id, { sort: row.sort })]);
...
const { columns, data, loading, getData, getDataByPage } = useUIPaginatedTable({
  api: async () => {
    ...
    if (!error && data && Array.isArray(data)) {
      ...
      return { data: displayData, list: cleanedTree, page: 1, pageSize: cleanedTree.length, total: cleanedTree.length };
    }
...
<template>
    <div class="page-container menu-management-page">
        ...
        <!-- 内容卡片 -->
        <div class="content-card menu-content-card" :style="{
        ...
        padding: 'var(--sx-content-card-padding)'
            ...
            <div class="workbench-toolbar workbench-toolbar--history menus-toolbar">
                <div class="workbench-toolbar-left">
                    <ElInput v-model="filterText" placeholder="搜索菜单名称 / 路径 / 权限" clearable style="width: 300px" @input="handleFilterChange" @clear="handleFilterChange">
                        <template #prefix>
                            <ElIcon>
                                <Search />
                            </ElIcon>
            ...
            <div class="table-section">
                <ElTable :key="tableKey" v-loading="loading" :data="data" border class="data-table" row-key="id" :tree-props="{ children: 'children', indent: 20 }">
