...
<template>
  ...
  >
    <ElAlert
      ...
      show-icon
      class="mb-16px"
      title="每个 API 端点必须配置至少一个权限码，未配置的端点将被拒绝访问（fail-closed）。一个权限码可保护多个端点；同一端点也可配置多个权限码（用户持有任一即可访问），用于实现「单接口权限」与「接口集合权限」两种语义并存。"
    />
    ...
    <ElForm ref="formRef" inline :model="addModel" :rules="addRules" class="mb-12px">
      <ElFormItem prop="permissionCode">
        <ElSelect
          v-model="addModel.permissionCode"
          placeholder="选择权限编码"
          filterable
          style="width: 260px"
        >
    ...
    <ElDialog v-model="editVisible" title="修改归属权限码" width="440px" append-to-body>
      <ElForm label-position="top">
        ...
        <ElFormItem label="目标权限编码" required>
          <ElSelect
            v-model="editModel.permissionCode"
            placeholder="选择权限编码"
            filterable
            style="width: 100%"
          >
