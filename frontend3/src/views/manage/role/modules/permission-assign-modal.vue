...
<template>
    <ElDialog v-model="visible" :title="title" width="900px" :close-on-click-modal="false">
        <div v-loading="loading" class="permission-assign-container">
            <div class="tip-text">
                <span>勾选权限节点，为角色分配相应的操作权限。权限按模块 → 页面 → 按钮层级组织。</span>
            </div>
            ...
            <ElTree ref="treeRef" :data="permissionTree" node-key="id" show-checkbox :props="{
          label: 'label',
          children: 'children'
        }" class="permission-tree" :default-expand-all="true" :indent="24" @check-change="handleCheckChange">
