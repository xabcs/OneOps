<script setup lang="ts">
    ...
    const title = computed(() =>
      props.viewOnly ? `查看角色 "${props.roleData?.name || ''}" 的权限` : `为角色 "${props.roleData?.name || ''}" 分配权限`
    );
...
<template>
    <ElDialog v-model="visible" :title="title" width="900px" :close-on-click-modal="false">
        <div v-loading="loading" class="permission-assign-container">
            ...
            <ElTree ref="treeRef" :data="permissionTree" node-key="id" :show-checkbox="!props.viewOnly" :props="{
          label: 'label',
          children: 'children'
