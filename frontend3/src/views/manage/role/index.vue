...
async function handleBatchDelete() {
  await executeWithPermission('system.role.batch_delete', async () => {
    if (checkedRowKeys.value.length === 0) {
      ElNotification({ title: '提示', message: '请选择要删除的角色', type: 'warning', duration: 3000, position: 'top-right' });
      return;
    }
    ...
    if (cannotDeleteRoles.length > 0) {
      ...
      ElNotification({ title: `无法删除 ${cannotDeleteRoles.length} 个角色`, message, type: 'warning', duration: 5000, position: 'top-right' });
    ...
    if (successCount > 0) {
      ElNotification({ title: '批量删除完成', message: `成功删除 ${successCount} 个角色`, type: 'success', duration: 3000, position: 'top-right' });
    }

    if (failCount > 0) {
      ElNotification({ title: '部分删除失败', message: `${failCount} 个角色删除失败`, type: 'error', duration: 5000, position: 'top-right' });
    }
...
async function handleDelete(id: number) {
  await executeWithPermission('system.role.delete', async () => {
    ...
    const { canDelete, reason } = canDeleteRole(role, roleUsersMap.value);
    if (!canDelete) {
      ElNotification({ title: '无法删除角色', message: reason || '该角色不可删除', type: 'warning', duration: 3000, position: 'top-right' });
      return;
    }
    ...
    if (!error) {
      ElNotification({ title: '删除成功', message: `角色 "${role.name}" 已成功删除`, type: 'success', duration: 3000, position: 'top-right' });
      onDeleted();
      await getAllUsers();
    } else {
      ElNotification({ title: '删除失败', message: error.msg || '删除角色失败', type: 'error', duration: 3000, position: 'top-right' });
    }
  });
}
...
<template>
    <div class="page-container role-management-page">
        ...
        <!-- 内容卡片 -->
        <div class="content-card role-content-card" :style="{
        ...
        padding: 'var(--sx-content-card-padding)'
            ...
            <div class="workbench-toolbar workbench-toolbar--history roles-toolbar">
                <div class="workbench-toolbar-left">
                    <ElInput v-model="searchParams.name" placeholder="搜索角色名称" clearable style="width: 200px" @input="handleSearchInput" />
                    <ElInput v-model="searchParams.code" placeholder="搜索角色编码" clearable style="width: 200px" @input="handleSearchInput" />
                </div>
        ...
        <PermissionAssignModal v-model:visible="permissionModalVisible" :role-id="currentRoleId" :role-data="currentRoleData" :view-only="isViewMode" @submitted="handlePermissionSubmitted" />
    </div>
</template>
