    ...
    <div class="flex flex-col gap-16px">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-12px">
                    ...
                    <div class="flex flex-col gap-2px">
                        <h2 class="text-18px font-bold m-0">用户组管理</h2>
                        <p class="text-13px opacity-70 m-0">平台用户组用于集群等资源的批量授权，组内成员自动继承组的集群权限</p>
                    </div>
                </div>
        ...
        <ElCard shadow="hover">
            ...
            <ElSpace wrap class="mb-16px">
                <ElInput v-model="searchParams.keyword" placeholder="搜索名称/编码" clearable style="width: 220px" :prefix-icon="Search" @input="handleSearchInput" />
            ...
            <!-- 分页 -->
            <div v-if="total" class="flex justify-end mt-16px">
                <ElPagination layout="total, sizes, prev, pager, next, jumper" :total="total" :current-page="searchParams.page" :page-size="searchParams.pageSize" @current-change="handlePageChange" @size-change="handleSizeChange" />
            </div>
        </ElCard>

        <!-- 抽屉和弹窗 -->
        <UserGroupOperateDrawer v-model:visible="drawerVisible" :operate-type="operateType" :row-data="editingData" @submitted="getData" />
