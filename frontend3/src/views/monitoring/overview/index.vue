<script setup lang="ts">
    ...
    const overview = ref<Monitoring.MonitoringOverview | null>(null);
    ...
    // WebSocket 实时更新处理
    function handleOverviewUpdate(data: Record<string, unknown>) {
      if (data && overview.value) {
        overview.value = { ...overview.value, ...data };
      }
    }

    function handleAlertUpdate(alert: Monitoring.MonitoringOverview['activeAlerts'][number]) {
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs text-gray-400">
                        <span class="mr-1 inline-block h-2 w-2 rounded-full" :class="wsClient?.isConnected ? 'bg-green-500' : 'bg-red-500'"></span>
                    ...
                    <ElRow :gutter="16">
                        <ElCol :xs="24" :lg="8">
                            <ElCard shadow="never" header="Top 10 CPU 使用率">
                                <div class="top-list">
                                    <div v-for="(item, index) in (overview.topCpu || []).slice(0, 10)" :key="item.serverId" class="top-list-item">
                                        ...
                                        <div class="top-list-value">
                                            <ElProgress :percentage="Math.round(item.cpuUsage || 0)" :color="getProgressColor(item.cpuUsage || 0)" :stroke-width="8" :show-text="true" :format="() => (item.cpuUsage || 0).toFixed(1) + '%'" />
                                        </div>
                                    </div>
                                </div>
                            </ElCard>
                        ...
                        <ElCol :xs="24" :lg="8">
                            <ElCard shadow="never" header="Top 10 内存使用率">
                                <div class="top-list">
                                    <div v-for="(item, index) in (overview.topMemory || []).slice(0, 10)" :key="item.serverId" class="top-list-item">
                                        <div class="top-list-rank" :class="`rank-${index + 1}`">{{ index + 1 }}</div>
                                        <div class="top-list-content">
                                            <div class="top-list-title">{{ item.hostname }}</div>
                                            <div class="top-list-subtitle">{{ item.ip }}</div>
                                        ...
                                        <div class="top-list-value">
                                            <ElProgress :percentage="Math.round(item.memoryUsage || 0)" :color="getProgressColor(item.memoryUsage || 0)" :stroke-width="8" :show-text="true" :format="() => (item.memoryUsage || 0).toFixed(1) + '%'" />
                                        </div>
                                    </div>
                                </div>
                            </ElCard>
                        ...
                        <ElCol :xs="24" :lg="8">
                            <ElCard shadow="never" header="Top 10 磁盘使用率">
                                <div class="top-list">
                                    <div v-for="(item, index) in (overview.topDisk || []).slice(0, 10)" :key="item.serverId" class="top-list-item">
                                        <div class="top-list-rank" :class="`rank-${index + 1}`">{{ index + 1 }}</div>
                                        <div class="top-list-content">
                                            <div class="top-list-title">{{ item.hostname }}</div>
                                            <div class="top-list-subtitle">{{ item.ip }}</div>
                                        ...
                                        <div class="top-list-value">
                                            <ElProgress :percentage="Math.round(item.diskUsage || 0)" :color="getProgressColor(item.diskUsage || 0)" :stroke-width="8" :show-text="true" :format="() => (item.diskUsage || 0).toFixed(1) + '%'" />
                                        </div>
                                    </div>
                                </div>
                            </ElCard>
