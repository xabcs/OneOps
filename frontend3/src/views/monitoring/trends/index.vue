<script setup lang="ts">
    ...
    const chartData = ref<Monitoring.MetricsDatapoint[]>([]);
      ...
      const option: echarts.EChartsOption = {
        ...
        tooltip: {
          ...
          formatter: (params: { name: string; seriesName: string; value: number }[]) => {
            ...
            return `${param.name}<br/>${param.seriesName}: ${param.value.toFixed(2)}${getMetricUnit(
              filterForm.metricType
            )}`;
          }
        },
        grid: {
    ...
    <div class="p-4 space-y-4">
        ...
        <ElCard shadow="never">
            <ElForm :model="filterForm" inline>
                <ElFormItem label="主机">
                    <ElSelect v-model="filterForm.serverId" placeholder="选择主机" filterable style="width: 200px" @change="handleServerChange">
                        <ElOption v-for="server in servers" :key="server.id" :label="`${server.hostname} (${server.ip})`" :value="server.id" />
                    </ElSelect>
                </ElFormItem>
                ...
                <ElFormItem label="自定义时间">
                    <ElDatePicker v-model="customDateRange" type="datetimerange" range-separator="至" start-placeholder="开始时间" end-placeholder="结束时间" format="YYYY-MM-DD HH:mm" value-format="YYYY-MM-DD HH:mm:ss" @change="handleCustomDateChange" />
                </ElFormItem>
            </ElForm>
        </ElCard>
