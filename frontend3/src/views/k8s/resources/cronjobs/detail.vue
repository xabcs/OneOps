<script setup lang="ts">
    ...
    async function loadData() {
      loading.value = true;
      try {
        const response = await getK8sCronJob(clusterId.value, namespace.value, resourceName.value);
    ...
    async function loadPods() {
      ...
      try {
        const response = await getK8sCronJobPods(clusterId.value, namespace.value, resourceName.value);
    ...
    <div class="detail-page">
        ...
        <ElTabs v-model="activeTab" class="detail-tabs" @tab-change="handleTabChange">
            ...
            <ElTabPane :label="`事件 (${events.length})`" name="events">
                ...
                <div class="tab-content">
                    <ElTable v-loading="eventsLoading" :data="events" class="cronjobs-events-table" :header-cell-style="{
              background: '#f5f7fa',
              color: '#303133',
              fontWeight: '600',
              paddingLeft: '16px',
...
<style scoped>
    ...
    .cronjobs-events-table,
    .cronjobs-events-table.el-table,
    :deep(.el-table),
    :deep(.el-table__body),
