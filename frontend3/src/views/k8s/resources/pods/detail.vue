<script setup lang="ts">
    ...
    const resource = ref<K8s.Pod | null>(null);
    ...
    async function loadDetail() {
      ...
      try {
        ...
        ElMessage.error(err.message || '获取 Pod 详情失败');
      } finally {
        loading.value = false;
      }
    }
    ...
    function handleTerminal(containerName?: string) {
      ...
      const terminalUrl = `${baseUrl}/k8s/terminal?clusterId=${clusterId.value}&namespace=${namespace.value}&podName=${resourceName.value}&containerName=${cName}`;

      // 强制在新标签页打开
      const newWindow = window.open(terminalUrl, '_blank');

      if (!newWindow) {
        console.error('❌ [终端] 浏览器阻止了新标签页打开');
...
<template>
    <div v-loading="loading" class="detail-page bg-layout">
        <!-- 顶部操作栏 - 使用新组件 -->
        <K8sResourceActionBar :name="resource?.name || '-'" :namespace="resource?.namespace || '-'" :status-tag="getPhaseTag" :back-path="backPath" :actions="[
                    ...
                    <div v-if="resource?.selectors && Object.keys(resource.selectors).length > 0" class="desc-row">
                        <div class="desc-item desc-item-full">
                            ...
                            <div class="item-content-vertical selectors-list">
                                <ElTag v-for="(selector, idx) in formatSelectorsList(resource.selectors)" :key="idx" type="primary" size="small" class="selector-tag">
                    ...
                    <div v-if="resource?.labels && Object.keys(resource.labels).length > 0" class="desc-row">
                        <div class="desc-item desc-item-full">
                            ...
                            <div class="item-content tags-list">
                                <ElTag v-for="(tag, idx) in formatLabels(resource.labels)" :key="idx" :type="getTagType(tag.key)" size="small" class="tag-item">
                                    {{ tag.key }}: {{ tag.value }}
                                </ElTag>
                            </div>
                        </div>
        ...
        <ElTabs v-model="activeTab" class="detail-tabs" @tab-change="handleTabChange">
            ...
            <ElTabPane :label="`容器 (${resource?.containers?.length || 0})`" name="containers">
                ...
                <div class="tab-content">
                    <div v-if="resource?.containers && resource.containers.length > 0">
                        <ElTable :data="resource.containers" stripe size="small" class="pods-containers-table" :header-cell-style="{
                background: '#f5f7fa',
                color: '#303133',
                fontWeight: '600',
                paddingLeft: '16px',
            ...
            <ElTabPane :label="`事件 (${events.length})`" name="events">
                ...
                <div class="tab-content">
                    <ElTable v-loading="eventsLoading" :data="events" stripe size="small" class="pods-events-table" :header-cell-style="{ background: '#f5f7fa', color: '#303133', fontWeight: '600' }" :row-style="{ backgroundColor: 'transparent' }" :cell-style="{ backgroundColor: 'transparent', padding: '8px 0' }">
        ...
        <!-- 日志对话框 -->
        <ElDialog v-model="showLogs" :title="`日志: ${logPodName}${logContainerName ? ' (' + logContainerName + ')' : ''}`" width="900px" top="5vh">
