<script setup lang="ts">
    ...
    const resource = ref<K8s.Service | null>(null);
    const events = ref<K8s.Event[]>([]);
    const ports = ref<K8s.ServicePort[]>([]);
...
<template>
    <div class="detail-page bg-layout">
        <!-- 顶部操作栏 - 使用新组件 -->
        <K8sResourceActionBar :name="resourceName || '-'" :namespace="namespace || '-'" :status-tag="{ type: 'info', text: 'Service' }" :back-path="backPath" :actions="[
