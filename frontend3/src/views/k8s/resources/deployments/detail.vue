...
<template>
    <div v-loading="loading" class="detail-page bg-layout">
        <!-- 顶部操作栏 - 使用新组件 -->
        <K8sResourceActionBar :name="deployment?.name || '-'" :namespace="deployment?.namespace || '-'" :status-tag="getStatusTag" :back-path="backPath" :actions="[
        ...
        <!-- YAML 编辑器 -->
        <YamlEditor v-model="showYamlEditor" :title="`编辑 ${deploymentName}`" :yaml="yamlContent" :can-edit="true" :on-apply="handleYamlApply" />
    </div>
</template>
