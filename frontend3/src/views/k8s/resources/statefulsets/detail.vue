<script setup lang="ts">
    ...
    const resource = ref<K8s.StatefulSet | null>(null);
...
<template>
    <div v-loading="loading" class="detail-page bg-layout">
        <!-- 顶部操作栏 - 使用新组件 -->
        <K8sResourceActionBar :name="resource?.name || '-'" :namespace="resource?.namespace || '-'" :status-tag="getStatusTag" :back-path="backPath" :actions="[{ label: 'YAML 编辑', handler: handleEditYaml, tooltip: '编辑 YAML 配置' }]" />
