<script setup lang="ts">
  ...
  const dataSource = ref<K8s.ClusterUser[]>([]);

  const loadUsers = async () => {
    loading.value = true;
    try {
      const { data, error } = await fetchK8sClusterUsers(props.clusterId);
      if (!error && data) {
  ...
  const handleRevoke = async (row: K8s.ClusterUser) => {
    try {
      ...
      if (error !== 'cancel') {
        const err = error as Error;
        ElMessage.error(err.message || '撤销权限失败');
      }
    }
  };
