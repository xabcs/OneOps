  ...
  <div class="flex flex-col gap-16px">
    ...
    <ElDrawer v-model="detailVisible" :title="detailTitle" size="60%">
      <div v-loading="detailLoading">
        ...
        <template v-if="roleDetail">
          <ElAlert
            v-if="roleDetail.managedBy"
            type="warning"
            show-icon
            :closable="false"
            :title="`该对象由 ${roleDetail.managedBy} 管理，在此编辑可能在下次同步/升级时被还原`"
            class="mb-12px"
          />
          <div class="mb-12px flex items-center gap-8px">
            ...
            <PermissionButton code="k8s.rbac.manage" type="primary" size="small" class="ml-auto" @click="openEdit">
              编辑 rules
            </PermissionButton>
          </div>
          <ElTable :data="roleDetail.rules" stripe border>
            <ElTableColumn label="API Groups" min-width="140">
              <template #default="{ row }">
                <ElTag v-for="g in row.apiGroups" :key="g" size="small" class="mb-2px mr-4px">{{ g || 'core' }}</ElTag>
              </template>
            </ElTableColumn>
            ...
            <ElTableColumn label="Verbs" min-width="220">
              <template #default="{ row }">
                <ElTag v-for="v in row.verbs" :key="v" size="small" type="success" effect="plain" class="mb-2px mr-4px">
                  {{ v }}
                </ElTag>
              </template>
            </ElTableColumn>
            <ElTableColumn label="NonResourceURLs" min-width="140">
              <template #default="{ row }">
                <template v-if="row.nonResourceURLs && row.nonResourceURLs.length">
                  <ElTag
                    v-for="u in row.nonResourceURLs"
                    :key="u"
                    size="small"
                    type="warning"
                    effect="plain"
    ...
    <ElDialog v-model="editVisible" :title="`编辑 ${editKind} - ${editName}`" width="720px">
      <ElAlert
        ...
        show-icon
        :closable="false"
        :title="`该对象由 ${editManagedBy} 管理，保存的修改可能在下次同步/升级时被还原`"
        class="mb-12px"
      />
      <ElAlert
        v-else
        ...
        show-icon
        :closable="false"
        title="以 manifest 整体替换 rules（apiVersion/kind/metadata.name 需保留），保存前请确认 JSON 有效"
        class="mb-12px"
      />
