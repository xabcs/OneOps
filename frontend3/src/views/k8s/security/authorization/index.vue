{"code":200,"success":true,"message":"扩缩容 Deployment 成功"}<script setup lang="ts">
    ...
    const rowNsDisabled = (row: PendingPermRow) =>
      row.clusterId === undefined || row.clusterId === 0 || (row.permission === 'custom' && row.roleKind === 'ClusterRole');
...
<template>
    <div class="flex flex-col gap-16px">
        <ElAlert type="info" show-icon :closable="false" title="以用户/用户组视角管理集群访问授权：有效权限 = 系统权限码 ∧ 集群内原生 RBAC 绑定（含经用户组继承）；此处创建集群内真实 Binding，操作经 impersonation 由 kube-apiserver 最终判定" class="!mb-0" />

        <ElTabs v-model="activeTab">
            <!-- 用户 tab：主体视角 -->
            <ElTabPane label="用户" name="users">
                <div class="flex items-center gap-8px mb-12px">
                    <ElInput v-model="userSearch" :prefix-icon="Search" placeholder="按用户名 / 昵称搜索" clearable style="width: 240px" />
                </div>
                <ElTable v-loading="loading && users.length === 0" :data="filteredUsers" stripe>
                    <ElTableColumn label="用户" min-width="200">
                        <template #default="{ row }">
                            <span>{{ row.nickname || row.username }}</span>
                            <span v-if="row.nickname && row.nickname !== row.username" class="text-12px opacity-60 ml-4px">{{ row.username }}</span>
                        </template>
                    </ElTableColumn>
                    ...
                    <ElTableColumn label="有效授权" width="170">
                        <template #default="{ row }">
                            ...
                            <span v-if="userPermSummary(row).inherited > 0" class="text-12px opacity-60 ml-4px">
                    ...
                    <ElTableColumn label="操作" width="110" fixed="right">
                        <template #default="{ row }">
                            <ElButton link type="primary" size="small" @click="openPerms({ type: 'user', id: row.id, label: row.nickname || row.username, sub: row.username, groupIds: row.groupIds })">
                                权限管理
                            </ElButton>
                        </template>
                    </ElTableColumn>
            ...
            <ElTabPane label="用户组" name="groups">
                <div class="flex items-center gap-8px mb-12px">
                    <ElInput v-model="groupSearch" :prefix-icon="Search" placeholder="按名称 / 编码搜索" clearable style="width: 240px" />
                </div>
                <ElTable v-loading="loading && groups.length === 0" :data="filteredGroups" stripe>
                    <ElTableColumn label="用户组" min-width="200">
                        <template #default="{ row }">
                            <span>{{ row.name }}</span>
                            <span v-if="row.code && row.code !== row.name" class="text-12px opacity-60 ml-4px">{{ row.code }}</span>
                        </template>
                    </ElTableColumn>
                    ...
                    <ElTableColumn label="操作" width="110" fixed="right">
                        <template #default="{ row }">
                            <ElButton link type="primary" size="small" @click="openPerms({ type: 'group', id: row.id, label: row.name, sub: row.code || '' })">
                                权限管理
                            </ElButton>
                        </template>
                    </ElTableColumn>
                ...
                <ElTable v-loading="loading" :data="filteredRecords" stripe>
                    ...
                    <ElTableColumn label="主体" min-width="150">
                        <template #default="{ row }">
                            ...
                            <span v-if="row.subjectNickname && row.subjectNickname !== row.subjectName" class="text-12px opacity-60 ml-4px">
                                {{ row.subjectNickname }}
                            </span>
                        </template>
                    </ElTableColumn>
        ...
        <ElDrawer v-model="permsVisible" title="权限管理" size="70%">
            ...
            <ElText type="info" size="small" class="block mb-8px">
            ...
            <ElTable :data="permRows" stripe size="small" class="mb-8px">
                <ElTableColumn label="来源" width="140">
                    <template #default="{ row }">
                        <ElTag :type="row.inherited ? 'warning' : 'success'" size="small" effect="plain">{{ row.sourceLabel }}</ElTag>
                    </template>
                </ElTableColumn>
                ...
                <ElTableColumn v-if="canRevoke" label="操作" width="90" fixed="right">
                    <template #default="{ row }">
                        <ElButton v-if="!row.inherited" type="danger" link size="small" @click="handleRevoke(row.binding)">撤销</ElButton>
            ...
            <template v-if="canAssign">
                ...
                <ElTable :data="pendingRows" size="small" class="mb-8px">
                    <ElTableColumn label="集群" min-width="170">
                        <template #default="{ row }">
                            <ElSelect v-model="row.clusterId" filterable placeholder="选择集群" style="width: 100%" @change="onRowClusterChange(row)">
                    ...
                    <ElTableColumn label="命名空间" min-width="200">
                        <template #default="{ row }">
                            <ElSelect v-model="row.namespaces" multiple filterable collapse-tags :disabled="rowNsDisabled(row)" :placeholder="rowNsPlaceholder(row)" style="width: 100%">
                                <ElOption v-for="ns in row.nsOptions" :key="ns" :value="ns" :label="ns" />
                            </ElSelect>
                            <ElText v-if="!rowNsDisabled(row) && row.namespaces.length > 0 && row.namespaces.length < row.nsOptions.length" type="info" size="small" class="block">
                    ...
                    <ElTableColumn label="权限" min-width="130">
                        <template #default="{ row }">
                            <ElSelect v-model="row.permission" filterable placeholder="选择权限" style="width: 100%" @change="onRowPermissionChange(row)">
                    ...
                    <ElTableColumn label="自定义角色" min-width="230">
                        <template #default="{ row }">
                            <template v-if="row.permission === 'custom'">
                                ...
                                <ElSelect v-model="row.roleName" filterable placeholder="选择集群内角色" style="width: 100%; margin-top: 4px">
            ...
            <template #footer>
                <ElButton type="primary" :loading="submitting" :disabled="!canAssign || pendingRows.length === 0" @click="handleSubmitPerms">
                    提交授权
                </ElButton>
