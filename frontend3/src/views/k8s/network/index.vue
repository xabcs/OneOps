...
<template>
    <div class="network-page p-24px">
        ...
        <ElTabs v-model="activeTab" @tab-change="handleTabChange">
            ...
            <ElTabPane label="服务" name="services">
                <div v-loading="loading" class="table-container">
                    <ElTable :data="servicesData" class="network-table" :header-cell-style="{ background: '#f5f7fa', color: '#303133', fontWeight: '600' }" :row-style="{ backgroundColor: 'transparent' }" :cell-style="{ backgroundColor: 'transparent', padding: '8px 0' }" @selection-change="handleSelectionChange" @select-all="handleSelectAll">
                    ...
                    <div class="bottom-toolbar">
                        ...
                        <ElPagination v-model:current-page="servicesPagination.page" v-model:page-size="servicesPagination.pageSize" :total="servicesPagination.itemCount" :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" @current-change="handlePageChange" @size-change="handlePageSizeChange" />
                    </div>
                </div>
            </ElTabPane>
            ...
            <ElTabPane label="路由" name="ingresses">
                <div v-loading="loading" class="table-container">
                    <ElTable :data="ingressesData" class="network-table" :header-cell-style="{ background: '#f5f7fa', color: '#303133', fontWeight: '600' }" :row-style="{ backgroundColor: 'transparent' }" :cell-style="{ backgroundColor: 'transparent', padding: '8px 0' }" @selection-change="handleSelectionChange" @select-all="handleSelectAll">
                        <ElTableColumn type="selection" width="50" align="center" />
                        <ElTableColumn prop="name" label="名称" min-width="180" align="left">
                            <template #default="{ row }">
                                <ElButton link type="primary" @click="goToDetail(row)">{{ row.name }}</ElButton>
                    ...
                    <div class="bottom-toolbar">
                        ...
                        <ElPagination v-model:current-page="ingressesPagination.page" v-model:page-size="ingressesPagination.pageSize" :total="ingressesPagination.itemCount" :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" @current-change="handlePageChange" @size-change="handlePageSizeChange" />
                    </div>
                </div>
            </ElTabPane>
        </ElTabs>
        ...
        <!-- YAML 编辑器 -->
        <YamlEditor v-model="showYamlDialog" :title="yamlDialogTitle" :yaml="yamlContent" :can-edit="true" :on-apply="handleYamlApply" />
    </div>
</template>
