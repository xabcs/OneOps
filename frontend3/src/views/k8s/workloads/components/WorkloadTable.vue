...
<template>
    <div v-loading="loading" class="table-container">
        <ElTable :data="data" class="workloads-table" :header-cell-style="headerCellStyle" :row-style="rowStyle" :cell-style="cellStyle" @selection-change="emit('selection-change', $event)" @select-all="emit('select-all', $event)">
        ...
        <div class="bottom-toolbar">
            <div class="batch-operations-inline">
                ...
                <div class="batch-actions">
                    <template v-for="btn in batchButtons" :key="btn.label">
                        <ElButton size="default" :type="(btn.type as 'primary' | 'success' | 'warning' | 'danger' | 'info' | 'default' | undefined)" :disabled="btn.disabled" @click="btn.handler">
                            {{ btn.label }}
                        </ElButton>
                    </template>
            ...
            <ElPagination v-model:current-page="pagination.page" v-model:page-size="pagination.pageSize" :total="pagination.itemCount" :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" @current-change="emit('page-change', $event)" @size-change="emit('size-change', $event)" />
        </div>
    </div>
</template>
