<script setup lang="ts">
  ...
  async function handleTerminate(session: Bastion.BastionSession) {
    ...
    try {
      ...
      if (err?.response?.status === 401 || err?.response?.status === 404) {
        ...
        window.$message?.error(err.message || '断开会话失败');
      }
    }
  }
  ...
  async function handleCleanupInvalidSessions() {
    ...
    sessions.value = sessions.value.filter(session => {
      ...
      return result?.status === 'fulfilled' && (result.value as { value: { valid: boolean } }).value.valid;
    });
...
<template>
  <ListPageLayout title="在线会话" description="实时监控堡垒机活跃会话，支持强制断开与无效会话清理">
    <!-- 提示 + 统计卡片 -->
    <template #hero>
      <ElAlert type="info" :closable="false" class="mb-16px">
        <template #default>
    ...
    <!-- 工具栏 -->
    <template #toolbar>
      <PermissionButton code="cmdb.session.terminate" type="warning" @click="handleCleanupInvalidSessions">
        清理无效会话
      </PermissionButton>
