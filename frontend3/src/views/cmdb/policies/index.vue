        ...
        <ElCard shadow="never">
            ...
            <ElTable v-loading="loading" :data="policies" stripe style="width: 100%">
                ...
                <ElTableColumn label="允许账号" width="200">
                    <template #default="{ row }">
                        <ElTag v-for="(account, idx) in (row.loginAccounts || []).slice(0, 2)" :key="idx" size="small" style="margin-right: 4px">
                            {{ account }}
                        </ElTag>
                ...
                <ElTableColumn label="允许协议" width="120">
                    <template #default="{ row }">
                        <ElTag v-for="(protocol, idx) in row.protocols || []" :key="idx" size="small" :type="protocol === 'ssh' ? 'primary' : 'success'" style="margin-right: 4px">
            ...
            <div class="pagination-wrapper">
                <ElPagination v-model:current-page="pagination.page" v-model:page-size="pagination.pageSize" :total="total" :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" @size-change="getPolicies" @current-change="getPolicies" />
            </div>
        </ElCard>

        <!-- 策略表单对话框 -->
        <ElDialog v-model="dialogVisible" :title="dialogMode === 'create' ? '新增策略' : '编辑策略'" width="700px" :close-on-click-modal="false" @close="handleCloseDialog">
            <ElForm ref="formRef" :model="currentPolicy" :rules="rules" label-width="120px">
                ...
                <ElFormItem label="授权对象类型" prop="subjectType">
                    <ElSelect v-model="currentPolicy.subjectType" placeholder="选择授权对象类型" style="width: 100%">
                        <ElOption v-for="option in subjectTypeOptions" :key="option.value" :value="option.value" :label="option.label" />
                    </ElSelect>
                </ElFormItem>
                ...
                <ElFormItem label="资产范围类型" prop="assetScopeType">
                    <ElSelect v-model="currentPolicy.assetScopeType" placeholder="选择资产范围类型" style="width: 100%">
                        <ElOption v-for="option in assetScopeTypeOptions" :key="option.value" :value="option.value" :label="option.label" />
                    </ElSelect>
                </ElFormItem>
                ...
                <ElFormItem v-if="currentPolicy.assetScopeType === 'group'" label="资产范围" prop="assetScopeId">
                    <ElTreeSelect v-model="currentPolicy.assetScopeId" :data="groupOptions" :props="{ label: 'name', value: 'id', children: 'children' }" placeholder="请选择主机分组" check-strictly style="width: 100%" />
                </ElFormItem>

                <!-- 资产范围为业务系统时 -->
                <ElFormItem v-if="currentPolicy.assetScopeType === 'business'" label="资产范围" prop="assetScopeId">
                    <ElTreeSelect v-model="currentPolicy.assetScopeId" :data="businessOptions" :props="{ label: 'name', value: 'id', children: 'children' }" placeholder="请选择业务系统" check-strictly style="width: 100%" />
                </ElFormItem>
                ...
                <ElFormItem label="允许的登录账号">
                    <div class="tags-input-wrapper">
                        <ElTag v-for="(account, idx) in currentPolicy.loginAccounts" :key="idx" closable style="margin-right: 8px; margin-bottom: 8px" @close="handleRemoveLoginAccount(idx)">
                            {{ account }}
                        </ElTag>
                        <ElInput v-if="!currentPolicy.loginAccounts || currentPolicy.loginAccounts.length === 0" placeholder="输入账号后按回车" size="small" style="width: 150px" @change="
                (val: string) => {
                  if (val) {
              ...
              " />
                ...
                <ElFormItem label="允许的协议">
                    <ElCheckboxGroup v-model="currentPolicy.protocols">
                        <ElCheckbox v-for="option in protocolOptions" :key="option.value" :value="option.value" :label="option.label" />
                    </ElCheckboxGroup>
                </ElFormItem>
                ...
                <ElFormItem label="高危命令">
                    <div class="tags-input-wrapper">
                        <ElTag v-for="(cmd, idx) in currentPolicy.highRiskCommands" :key="idx" closable type="danger" style="margin-right: 8px; margin-bottom: 8px" @close="handleRemoveHighRiskCommand(idx)">
                            {{ cmd }}
                        </ElTag>
