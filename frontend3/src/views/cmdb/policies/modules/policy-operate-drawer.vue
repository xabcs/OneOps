<script setup lang="ts">
    ...
    function buildPayload(): Bastion.AccessPolicyForm {
      return {
        ...
        allowFileTransfer: !!model.value.allowFileTransfer,
        allowSudo: !!model.value.allowSudo,
    ...
    <ElDrawer v-model="visible" :title="title" :width="700" destroy-on-close>
        <ElForm ref="formRef" :model="model" :rules="rules" label-width="120px">
            ...
            <ElFormItem v-else-if="model.assetScopeType === 'group'" label="资产范围" prop="assetScopeId">
                <ElTreeSelect v-model="model.assetScopeId" :data="serverGroups" :props="{ label: 'name', value: 'id', children: 'children' }" placeholder="请选择主机分组" check-strictly style="width: 100%" />
            </ElFormItem>

            <!-- 资产范围为业务系统时 -->
            <ElFormItem v-else-if="model.assetScopeType === 'business'" label="资产范围" prop="assetScopeId">
                <ElTreeSelect v-model="model.assetScopeId" :data="businessUnits" :props="{ label: 'name', value: 'id', children: 'children' }" placeholder="请选择业务系统" check-strictly style="width: 100%" />
            </ElFormItem>
            ...
            <ElFormItem label="登录账号" prop="loginAccounts">
                <div class="tags-input-wrapper">
                    <ElTag v-for="(account, idx) in model.loginAccounts" :key="account" closable style="margin-right: 8px; margin-bottom: 8px" @close="handleRemoveLoginAccount(idx)">
                        {{ account }}
                    </ElTag>
                    <ElInput v-if="accountInputVisible" ref="accountInputRef" v-model="accountInputValue" placeholder="输入账号后回车" size="small" style="width: 180px" @keyup.enter="handleAccountInputConfirm" @blur="handleAccountInputConfirm" />
            ...
            <ElFormItem label="高危命令">
                <div class="tags-input-wrapper">
                    <ElTag v-for="(cmd, idx) in model.highRiskCommands" :key="cmd" closable type="danger" style="margin-right: 8px; margin-bottom: 8px" @close="handleRemoveHighRiskCommand(idx)">
                        {{ cmd }}
                    </ElTag>
                    <ElInput v-if="commandInputVisible" ref="commandInputRef" v-model="commandInputValue" placeholder="输入命令后回车" size="small" style="width: 180px" @keyup.enter="handleCommandInputConfirm" @blur="handleCommandInputConfirm" />
                ...
                <div class="time-window-wrapper">
                    ...
                    <template v-if="timeEnabled">
                        <div class="time-window-row">
                            ...
                            <ElTimeSelect v-model="timeWindow.end" :start="timeWindow.start || '00:00'" end="23:30" step="00:30" placeholder="结束时间" />
                        </div>
                        <div class="time-window-row">
