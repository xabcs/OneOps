<script setup lang="ts">
    ...
    import { fetchGetSessions, fetchCheckConnectPermission } from '@/service/api';
...
<template>
    <ElDrawer :model-value="visible" direction="rtl" size="820px" :title="server?.hostname || '主机详情'" destroy-on-close @close="emit('update:visible', false)">
        <div v-if="server">
                    ...
                    <div class="overview-section">
                        ...
                        <ElDescriptions :column="2" border size="small">
                            ...
                            <ElDescriptionsItem label="环境">
                                <ElTag :type="getEnvDisplayInfo(server.env).type" size="small">{{ getEnvDisplayInfo(server.env).label }}</ElTag>
                            </ElDescriptionsItem>
                            <ElDescriptionsItem label="主机状态">
                                <ElTag :type="server.status === 1 ? 'success' : 'info'" size="small">{{ server.status === 1 ? '正常' : '停用' }}</ElTag>
                            </ElDescriptionsItem>
                        </ElDescriptions>
                    </div>
                    <div class="overview-section">
                    ...
                    <div class="overview-section">
                        ...
                        <ElDescriptions :column="2" border size="small">
                            ...
                            <ElDescriptionsItem label="所属分组">
                                <ElTag v-for="group in server.groups" :key="group.id" size="small" style="margin-right: 4px">{{ group.name }}</ElTag>
                                <span v-if="!server.groups?.length" class="text-gray-400">未分组</span>
                            </ElDescriptionsItem>
                            <ElDescriptionsItem label="标签" :span="2">
                                <ElTag v-for="tag in server.tags" :key="tag.id" size="small" :color="tag.color" style="margin-right: 4px">{{ tag.name }}</ElTag>
                    ...
                    <div class="overview-section">
                        ...
                        <ElDescriptions :column="2" border size="small">
                            <ElDescriptionsItem label="Agent 状态">
                                <ElTag v-if="server.agentStatus === 'running'" type="success" size="small">运行中 (v{{ server.agentVersion || '-' }})</ElTag>
                            ...
                            <ElDescriptionsItem label="连通性状态">
                                <ElTag :type="server.connectivityStatus === 'online' ? 'success' : server.connectivityStatus === 'offline' ? 'danger' : 'info'" size="small">
                                    {{ server.connectivityStatus === 'online' ? '在线' : server.connectivityStatus === 'offline' ? '离线' : '未知' }}
                                </ElTag>
                            </ElDescriptionsItem>
                ...
                <ElTabPane label="连接信息" name="connect">
                    <ElDescriptions :column="1" border>
                        ...
                        <ElDescriptionsItem label="绑定凭证">
                            <div v-if="server.credentials?.length">
                                <ElTag v-for="cred in server.credentials" :key="cred.id" type="success" size="small" style="margin-right: 4px; margin-bottom: 4px">{{ cred.name }}（{{ cred.username }}）</ElTag>
                            </div>
                            <ElTag v-else type="warning" size="small">未配置</ElTag>
                        </ElDescriptionsItem>
                        <ElDescriptionsItem label="可用凭证">
                            <div>
                                <ElTag v-for="cred in drawerPermission.credentials" :key="cred.id" size="small" style="margin-right: 4px; margin-bottom: 4px">{{ cred.name }}（{{ cred.username }}）</ElTag>
                    ...
                    <div style="margin-top: 16px">
                        <ElButton type="success" @click="handleConnect(server)">
                            <icon-mdi-console-line style="margin-right: 4px" />连接此主机
                        </ElButton>
                    </div>
                </ElTabPane>
                    ...
                    <div class="monitor-section">
                        ...
                        <div class="usage-cards">
                            <div class="usage-card">
                                ...
                                <div class="card-value" :style="{ color: getUsageColor(server.cpuUsage || 0) }">{{ Math.round(server.cpuUsage || 0) }}%</div>
                                <ElProgress :percentage="Math.round(server.cpuUsage || 0)" :color="getUsageColor(server.cpuUsage || 0)" :show-text="false" />
                                <div class="card-detail">{{ server.cpu || '-' }} 核</div>
                            </div>
                            <div class="usage-card">
                                <div class="card-label">内存</div>
                                <div class="card-value" :style="{ color: getUsageColor(server.memoryUsage || 0) }">{{ Math.round(server.memoryUsage || 0) }}%</div>
                                <ElProgress :percentage="Math.round(server.memoryUsage || 0)" :color="getUsageColor(server.memoryUsage || 0)" :show-text="false" />
                                <div class="card-detail">{{ server.memory ? ((server.memory * (server.memoryUsage || 0)) / 100).toFixed(1) + ' / ' + server.memory + ' GB' : '-' }}</div>
                            </div>
                            <div class="usage-card">
                                <div class="card-label">磁盘</div>
                                <div class="card-value" :style="{ color: getUsageColor(server.diskUsage || 0) }">{{ Math.round(server.diskUsage || 0) }}%</div>
                                <ElProgress :percentage="Math.round(server.diskUsage || 0)" :color="getUsageColor(server.diskUsage || 0)" :show-text="false" />
                    ...
                    <div class="monitor-section">
                        ...
                        <ElTable :data="formatDiskPartitionsForDrawer(server)" size="small" border>
                            ...
                            <ElTableColumn prop="usage" label="使用率" width="80" align="center">
                                <template #default="{ row }"><span :style="{ color: getUsageColor(row.usage) }">{{ Math.round(row.usage) }}%</span></template>
                            </ElTableColumn>
                        </ElTable>
                        <div v-if="!server.diskPartitions || server.diskPartitions.length === 0" class="py-12px text-center text-gray-400">
