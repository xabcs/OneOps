<script setup lang="tsx">
  ...
  // 凭证管理共享面板：SSH 与 Access 两个路由页复用，defaultTab 控制初始类型 Tab
  const props = withDefaults(defineProps<{ defaultTab?: CMDB.CredentialType | 'all' }>(), {
    defaultTab: 'all'
  });

  // 类型 Tab
  const activeTab = ref<CMDB.CredentialType | 'all'>(props.defaultTab);
