    ...
    <div class="min-h-500px flex-col gap-4">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between gap-4">
                <div class="permission-tabs-shell flex-1">
                    <button v-for="app in applications" :key="app.id" type="button" class="permission-tab-btn" :class="{ active: selectedAppId === app.id }" @click="
            ...
            ">
                ...
                <div class="permission-tabs-shell">
                    <button type="button" class="permission-tab-btn" :class="{ active: viewMode === 'matrix' }" @click="
              viewMode = 'matrix';
              handleViewModeChange();
            ">
                        矩阵视图
                    </button>
                    <button type="button" class="permission-tab-btn" :class="{ active: viewMode === 'list' }" @click="
              viewMode = 'list';
              handleViewModeChange();
            ">
                        列表视图
            ...
            <div v-if="viewMode === 'matrix'" class="permission-tabs-shell">
                ...
                <template v-if="selectedApp?.type === 'jenkins' || selectedApp?.type === 'gitlab'">
                    <button v-for="type in roleTypes" :key="type.value" type="button" class="permission-tab-btn" :class="{ active: selectedRoleType === type.value }" @click="
            ...
            ]" :key="type.value" type="button" class="permission-tab-btn" :class="{ active: selectedRoleType === type.value }" @click="
              selectedRoleType = type.value;
              handleRoleTypeChange();
            ">
                        {{ type.label }}
        ...
        <ElCard shadow="never" class="flex-1">
            ...
            <div v-if="viewMode === 'matrix'" v-loading="matrixLoading">
                ...
                <!-- 使用统一的矩阵组件 -->
                <AppPermissionMatrix v-else-if="adaptedMatrixData && adaptedMatrixData.columns && adaptedMatrixData.columns.length > 0" :data="adaptedMatrixData" :item-type="selectedRoleType" :app-type="selectedApp?.type || 'jenkins'" @refresh="getMatrixData" />
            ...
            <div v-else>
                ...
                <div class="mt-4 flex justify-end">
                    <ElPagination v-model:current-page="pagination.page" v-model:page-size="pagination.pageSize" :total="pagination.total" :page-sizes="[10, 20, 50, 100]" layout="total, sizes, prev, pager, next, jumper" @current-change="handlePageChange" @size-change="handleSizeChange" />
                </div>
            </div>
        </ElCard>
    </div>
