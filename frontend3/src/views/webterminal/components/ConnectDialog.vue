...
<template>
    <ElDialog :model-value="modelValue" title="连接主机" width="520px" :close-on-click-modal="true" :append-to-body="false" class="terminal-connect-dialog" @update:model-value="(val: boolean) => $emit('update:modelValue', val)" @close="handleClose">
        <div v-if="connectingServer" class="connect-dialog-content">
            ...
            <div class="connect-section">
                ...
                <div v-if="connectingServer.credentials && connectingServer.credentials.length > 0" class="credential-selector">
                    <ElSelect :model-value="selectedCredentialId" placeholder="请选择凭证" popper-class="terminal-select-dropdown" style="width: 100%" @update:model-value="handleCredentialChange">
