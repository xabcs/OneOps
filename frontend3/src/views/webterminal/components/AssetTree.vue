<script setup lang="ts">
    ...
    async function loadTreeData() {
      ...
      try {
        ...
        // 构建树节点
        const buildNode = (group: CMDB.ServerGroup): TreeNode => {
          const childGroups = (group.children || []).map((child: CMDB.ServerGroup) => buildNode(child));
          const groupServers = (group.servers || []).map((server: CMDB.Server) => ({
        ...
        const result = groups.map((g: CMDB.ServerGroup) => buildNode(g));
        ...
        if (ungroupedServers.length > 0) {
          result.unshift({
            ...
            name: 'Default',
            children: ungroupedServers.map((server: CMDB.Server) => ({
              id: `server-${server.id}` as unknown as number,
              name: `${server.hostname} (${server.ip})`,
              children: undefined,
              serverCount: undefined,
...
<template>
    <div class="wb-sidebar" @click="handleClickOutside">
                ...
                <Transition name="wb-search-expand">
                    ...
                    <div v-else key="search" class="wb-search-container">
                        <div class="wb-search-input-wrapper">
                            ...
                            <ElInput ref="searchInputRef" v-model="searchKeyword" placeholder="搜索..." size="small" clearable class="wb-search-input-inner" @click="handleSearchClick" />
                        </div>
                    </div>
                </Transition>
            </div>
        ...
        <div class="wb-sidebar-content">
            <ElTree ref="treeRef" :data="filteredTreeData" :props="{ label: 'name', children: 'children' }" :expand-on-click-node="false" node-key="id" :default-expand-all="false" class="wb-tree" @node-click="handleNodeClick" @node-dblclick="handleNodeDblClick">
                <template #default="{ node, data }">
                    <div class="wb-tree-node" :class="[getNodeClass(data), { selected: node.selected }]" @dblclick="handleNodeDblClick({ data })" @contextmenu.prevent="handleContextMenu(data, $event)">
                        ...
                        <span v-if="data.isLeaf && data.server && currentSessions.includes(Number(data.server.id))" class="wb-tag wb-tag-success">
                            已连接
                        </span>
                    </div>
                </template>
        ...
        <!-- 右键菜单 -->
        <Teleport to="body">
            <div v-if="contextMenuVisible" class="wb-dropdown-menu" :style="{
