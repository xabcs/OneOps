...
<template>
    <div class="terminal-workbench" :class="{ 'is-fullscreen': isFullscreen }">
        <!-- 活动栏 (Activity Bar) -->
        <ActivityBar v-if="showActivityBar && !isFullscreen" :show-sidebar="showSidebar && !isFullscreen" :is-fullscreen="isFullscreen" :active-activity-item="activeActivityItem" :server-menu-expanded="serverMenuExpanded" :server-menu-position="serverMenuPosition" :main-menu-expanded="mainMenuExpanded" :main-menu-position="mainMenuPosition" @toggle-sidebar="toggleSidebar" @toggle-fullscreen="toggleFullscreen" @toggle-server-menu="toggleServerMenu" @toggle-main-menu="toggleMainMenu" @switch-activity-item="switchActivityItem" />
        ...
        <!-- 连接对话框 -->
        <ConnectDialog v-model="showConnectDialog" :connecting-server="connectingServer" :selected-credential-id="selectedCredentialId" @update:selected-credential-id="selectedCredentialId = $event" @connect="handleConnected" @cancel="cancelConnect" />
    </div>
</template>
