<script setup lang="tsx">
      ...
      theme: (themeStore.darkMode ? VTable.themes.DARK : VTable.themes.DEFAULT).extends({
        groupTitleStyle: {
          ...
          bgColor: (args: {
            col: number;
            row: number;
            table: { getGroupTitleLevel: (col: number, row: number) => number | undefined };
          }) => {
  ...
  const handleLegendItemClick = (args: { value: string[] }) => {
    (
      pivotChartRef?.value as {
        vTableInstance: { updateFilterRules: (rules: { filterKey: string; filteredValues: string[] }[]) => void };
      }
    )?.vTableInstance.updateFilterRules([
      {
