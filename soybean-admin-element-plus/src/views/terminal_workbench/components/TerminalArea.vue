...
<template>
    <div class="wb-terminal-area">
        ...
        <div class="wb-terminal-content">
            ...
            <!-- 为所有会话渲染终端，使用 v-show 控制显示/隐藏 -->
            <div v-else class="wb-terminal-session">
                <div v-for="session in sessions" :key="session.id" class="wb-terminal-wrapper" :style="{ display: session.id === activeSession.id ? 'flex' : 'none' }">
                    <XTermTerminal :session-id="session.id" :server-id="session.serverId" :server-name="session.serverName" :server-ip="session.serverIp" :login-account="session.loginAccount" :websocket-url="session.websocketUrl" />
                </div>
            </div>
        </div>
    </div>
