<script setup lang="ts">
    ...
    const currentSessionIds = computed(() =>
        sessions.value.map((s: any) => s.serverId)
    );
    // 切换侧边栏显示
    ...
    async function handleConnected() {
        ...
        try {
            ...
            ElNotification.error({
                title: "连接失败",
                message:
                    error instanceof Error ? error.message : "连接服务器时发生错误",
            });
        }
    }
    ...
    function switchSession(sessionId: number) {
        activeSession.value =
            sessions.value.find((s: any) => s.id === sessionId) || null;
    }
    ...
    async function handleConnect(server: any) {
        ...
        try {
            ...
            if (response.data) {
                ...
                // 默认选中第一个凭证（如果有）
                if (
                    connectingServer.value.credentials &&
                    connectingServer.value.credentials.length > 0
                ) {
                    selectedCredentialId.value =
                        connectingServer.value.credentials[0].id;
                } else {
    ...
    <div class="terminal-workbench" :class="{ 'is-fullscreen': isFullscreen }">
        ...
        <div class="wb-main-content">
            ...
            <template v-else>
                ...
                <!-- 分割线 -->
                <div v-if="showSidebar && !isFullscreen" class="wb-sash" :class="{ 'wb-sash-resizing': isResizing }" @mousedown="startResize">
                    <div class="wb-sash-line"></div>
                </div>

                <!-- 右侧内容区 -->
                <div class="wb-content-area">
                    <SessionTabs :sessions="sessions" :active-id="activeSession?.id || null" :current-session-ids="currentSessionIds" :show-sidebar="showSidebar && !isFullscreen" @select="switchSession" @remove="removeSession" @connect="handleConnect" @toggle-sidebar="toggleSidebar" @toggle-fullscreen="toggleFullscreen" />
        ...
        <!-- 连接对话框 -->
        <ElDialog v-model="showConnectDialog" title="连接主机" width="520px" :close-on-click-modal="true" :append-to-body="false" class="terminal-connect-dialog" @close="cancelConnect">
                ...
                <div class="connect-section">
                    ...
                    <div v-if="connectingServer.credentials && connectingServer.credentials.length > 0" class="credential-selector">
                        <ElSelect v-model="selectedCredentialId" placeholder="请选择凭证" popper-class="terminal-select-dropdown" style="width: 100%;">
                            <ElOption v-for="cred in connectingServer.credentials" :key="cred.id" :label="`${cred.name} - ${cred.username}`" :value="cred.id">
...
<style lang="scss">
    ...
    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-dialog__header) {
    ...
    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-dialog__title) {
        color: #cccccc !important;
    ...
    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-dialog__footer) {
        background: #2d2d2d !important;
    ...
    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-tag.el-tag--success) {
        background: rgba(78, 201, 176, 0.1) !important;
        color: #4ec9b0 !important;
    }

    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-tag.el-tag--warning) {
        background: rgba(217, 119, 6, 0.1) !important;
        color: #d97706 !important;
    }

    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-tag.el-tag--info) {
        background: rgba(107, 114, 128, 0.1) !important;
        color: #6b7280 !important;
    }

    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-tag.el-tag--plain) {
    ...
    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-button--primary) {
    ...
    .terminal-workbench
        :deep(.el-dialog.terminal-connect-dialog .el-button--primary:hover) {
    ...
    .terminal-select-dropdown
        .el-select-dropdown__listbar::-webkit-scrollbar-track {
        background: #1e1e1e !important;
    }

    .terminal-select-dropdown
        .el-select-dropdown__listbar::-webkit-scrollbar-thumb {
        background: #555 !important;
        border-radius: 0 !important;
    }

    .terminal-select-dropdown
        .el-select-dropdown__listbar::-webkit-scrollbar-thumb:hover {
