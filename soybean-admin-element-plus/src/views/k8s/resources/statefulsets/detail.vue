...
const getStatusTag = computed(() => {
  ...
  if (total === 0) return { type: 'info', text: '未就绪' };
  if (ready === total) return { type: 'success', text: `运行中 (${ready}/${total})` };
  if (ready > 0) return { type: 'warning', text: `部分就绪 (${ready}/${total})` };
...
const basicInfoFields = computed(() => {
  ...
  const fields = [
    ...
    [
      { label: '就绪 / 期望副本', value: `${resource.value.ready || 0} / ${resource.value.replicas || 0}` },
      { label: '当前副本', value: resource.value.current?.toString() || '0' },
      { label: '已更新副本', value: resource.value.updated?.toString() || '0' }
    ]
  ];

  if (resource.value.serviceName) {
    fields[1].push({ label: 'Service 名称', value: resource.value.serviceName });
  }
...
async function loadDetail() {
  ...
  loading.value = true;
  try {
    const res = await getK8sStatefulSet(clusterId.value, namespace.value, resourceName.value);
...
async function loadPods() {
  ...
  try {
    const res = await getK8sStatefulSetPods(clusterId.value, namespace.value, resourceName.value);
...
async function loadEvents() {
  ...
  try {
    const fieldSelector = `involvedObject.name=${resourceName.value},involvedObject.kind=StatefulSet`;
