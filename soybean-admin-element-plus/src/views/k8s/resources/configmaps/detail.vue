...
async function loadData() {
  loading.value = true;
  try {
    const response = await getK8sConfigMap(clusterId.value, namespace.value, resourceName.value);
...
async function handleYamlApply(yamlStr: string) {
  ...
  try {
    ...
    await updateK8sConfigMap(clusterId.value, {
      namespace: namespace.value,
      manifest
    });
