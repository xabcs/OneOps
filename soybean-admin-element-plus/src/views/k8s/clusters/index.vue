<script setup lang="tsx">
	...
	import { Platform, Refresh, Plus, Delete, Search, RefreshRight } from '@element-plus/icons-vue';
	import {
		createK8sCluster,
		deleteK8sCluster,
		fetchK8sClusters,
...
<template>
	<div class="page-container cluster-management-page">
		...
		>
			...
			<div class="workbench-toolbar workbench-toolbar--history clusters-toolbar">
				<div class="workbench-toolbar-left">
					<el-select
						...
						@change="handleSearch"
					>
						<el-option label="标准集群" value="standard" />
						<el-option label="托管集群" value="managed" />
						<el-option label="边缘集群" value="edge" />
					</el-select>
				...
				<div class="workbench-toolbar-right">
					<el-button class="filter-refresh-btn" @click="handleResetSearch">
			...
			<div class="table-section">
				...
				>
					...
					<el-table-column prop="name" label="集群名称" min-width="140" show-overflow-tooltip>
						<template #default="{ row }">
							<span class="cluster-name-link">{{ row.name }}</span>
						</template>
					</el-table-column>
					<el-table-column prop="description" label="描述" min-width="120" show-overflow-tooltip />
					<el-table-column prop="endpoint" label="API 地址" min-width="200" show-overflow-tooltip />
					<el-table-column prop="clusterType" label="类型" width="100" />
					<el-table-column prop="region" label="区域" width="100" />
					<el-table-column prop="version" label="版本" width="80" show-overflow-tooltip />
					...
					<el-table-column label="操作" width="280" fixed="right" align="center">
						<template #default="{ row }">
							<div class="flex-center gap-8px">
								...
								<el-button link class="text-accent" @click="handleTestConnection(row)">测试</el-button>
								<el-popconfirm title="确定要删除集群吗？" @confirm="handleDelete(row)">
									<template #reference>
		...
		<!-- 创建/编辑集群对话框 -->
		<el-dialog v-model="dialogVisible" :title="modalTitle" width="600px" :close-on-click-modal="false">
			<el-form :model="form" label-width="100px">
				<el-form-item label="集群名称" required>
					<el-input v-model="form.name" placeholder="请输入集群名称" />
				</el-form-item>
				<el-form-item label="集群描述">
					<el-input v-model="form.description" type="textarea" placeholder="请输入集群描述" :rows="3" />
				</el-form-item>
				<el-form-item label="API 地址" required>
					<el-input v-model="form.endpoint" placeholder="https://k8s-api.example.com:6443" />
				</el-form-item>
				<el-form-item label="Kubeconfig" required>
					<el-input v-model="form.kubeconfig" type="textarea" placeholder="粘贴 kubeconfig 内容" :rows="10" />
				</el-form-item>
				<el-form-item label="集群类型" required>
					<el-select v-model="form.clusterType" placeholder="选择集群类型">
						<el-option label="标准集群" value="standard" />
						<el-option label="托管集群" value="managed" />
						<el-option label="边缘集群" value="edge" />
					</el-select>
				...
				<el-form-item label="区域">
					<el-input v-model="form.region" placeholder="如：us-west-2" />
				</el-form-item>
				<el-form-item label="节点数">
					<el-input-number v-model="form.nodeCount" :min="0" placeholder="自动获取" />
				</el-form-item>
			</el-form>
