...
// UserCreateRequest 用户创建请求（类型别名，实际定义在 adapter 包中）
type UserCreateRequest = appadapter.UserCreateRequest

// JenkinsSession Jenkins 会话信息（类型别名，实际定义在 adapter 包中）
type JenkinsSession = appadapter.JenkinsSession

// JenkinsCrumb CSRF Token 结构（类型别名，实际定义在 adapter 包中）
type JenkinsCrumb = appadapter.JenkinsCrumb
...
type ApplicationAdapter interface {
	...
	// FetchUsers 获取用户列表
	FetchUsers(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationUser, error)

	// FetchRoles 获取角色列表
	FetchRoles(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationRole, error)

	// FetchGroups 获取用户组列表（可选实现）
	FetchGroups(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationGroup, error)

	// CreateUser 在外部系统中创建用户，返回创建的用户ID
	CreateUser(baseURL string, authConfig map[string]interface{}, user *appadapter.UserCreateRequest) (string, error)
...
func NewAdapterFactory() *AdapterFactory {
	...
	// 注册内置适配器
	factory.RegisterAdapter("jenkins", &appadapter.JenkinsAdapter{})
	factory.RegisterAdapter("jumpserver", &appadapter.JumpserverAdapter{})
	factory.RegisterAdapter("gitlab", &appadapter.GitLabAdapter{})
	factory.RegisterAdapter("generic", &appadapter.GenericAdapter{})
