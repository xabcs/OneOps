...
func (g *GitLabAdapter) FetchUsers(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationUser, error) {
	// 获取认证信息
	...
	// 转换为 ApplicationUser
	users := make([]modelauth.ApplicationUser, 0, len(usersData))
...
func (g *GitLabAdapter) FetchRoles(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationRole, error) {
	// GitLab 的访问级别是预定义的
	roles := []modelauth.ApplicationRole{
		{
			RoleCode:    "10",
			RoleName:    "Guest",
			RoleType:    "project",
...
func (g *GitLabAdapter) FetchGroups(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationGroup, error) {
	// 获取认证信息
	token, ok := authConfig["token"].(string)
	if !ok || token == "" {
		return nil, fmt.Errorf("GitLab 需要认证 token")
	...
	result := make([]modelauth.ApplicationGroup, 0, len(groups))
	for _, group := range groups {
		result = append(result, modelauth.ApplicationGroup{
