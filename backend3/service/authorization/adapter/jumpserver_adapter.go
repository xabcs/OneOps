...
func (j *JumpserverAdapter) FetchUsers(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationUser, error) {
	// 获取端点配置（支持多种类型）
	...
	// 转换为 ApplicationUser
	users := make([]modelauth.ApplicationUser, 0, len(apiUsers))
	for _, user := range apiUsers {
		...
		users = append(users, modelauth.ApplicationUser{
...
func (j *JumpserverAdapter) FetchGroups(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationGroup, error) {
	// 获取端点配置（支持多种类型）
	...
	for _, group := range apiGroups {
		groups = append(groups, modelauth.ApplicationGroup{
			GroupCode:   group.ID,
...
func (j *JumpserverAdapter) FetchRoles(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationRole, error) {
	// JumpServer 的授权是通过授权规则（用户/用户组 -> 资产/节点）
	// 不需要同步角色，返回空列表
	logger.Info("JumpServer 不需要角色同步，授权通过授权规则管理")

	return []modelauth.ApplicationRole{}, nil
}
...
func (j *JumpserverAdapter) FetchAuthorizationRules(baseURL string, authConfig map[string]interface{}) ([]modelauth.ApplicationAuthorizationRule, error) {
	...
	return []modelauth.ApplicationAuthorizationRule{}, nil
}
...
func (j *JumpserverAdapter) parseAuthorizationRules(body []byte, url string) ([]modelauth.ApplicationAuthorizationRule, error) {
	...
	if err := json.Unmarshal(body, &apiRules); err != nil {
		...
		if err := json.Unmarshal(body, &wrappedResult); err != nil {
			...
			apiRules = make([]apiRule, 0, len(wrappedResult.Data))
			apiRules = append(apiRules, wrappedResult.Data...)
		}
	}
	...
	rules := make([]modelauth.ApplicationAuthorizationRule, 0)
	for i, apiRule := range apiRules {
		...
		rules = append(rules, modelauth.ApplicationAuthorizationRule{
			RuleID:      apiRule.ID,
