...
import (
	...
	authsvc "oneops/backend3/service/authorization"

	authctrl "oneops/backend3/controller/authorization"

	"github.com/gin-gonic/gin"
)

// SetupAuthorizationRoutes 设置授权中心相关路由
func SetupAuthorizationRoutes(r *gin.Engine) {
	// 创建 repository、services 和 controller
	authRepo := repoauth.NewAuthorizationRepository(database.GetDB())
	appPermSvc := authsvc.NewApplicationPermissionService(authRepo)
	applicationPermissionController := authctrl.NewApplicationPermissionController(appPermSvc)
