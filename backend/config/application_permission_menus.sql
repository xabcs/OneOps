...
-- 1. 添加主菜单：外部权限管理
INSERT INTO sys_menus (name, title, icon, path, component, redirect, order_num, parent_id, is_hidden, is_hidden_children, status, create_time, update_time)
...
-- 4. 添加子菜单：用户授权
INSERT INTO sys_menus (name, title, icon, path, component, redirect, order_num, parent_id, is_hidden, is_hidden_children, status, create_time, update_time)
VALUES ('application-permission-sys_users', '用户授权', 'mdi:account-key', '/application-permission/sys_users', 'view.application_permission', NULL, 3, @main_menu_id, 0, 0, 1, NOW(), NOW());
...
-- 查看插入的菜单
SELECT * FROM sys_menus WHERE name LIKE 'application-permission%' ORDER BY order_num;
