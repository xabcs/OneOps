...
-- 2. 创建主机属性值表
CREATE TABLE IF NOT EXISTS server_attributes (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY COMMENT '属性ID',
...
-- 业务系统
INSERT INTO attribute_definitions (name, `key`, category, type, options, sort_order, description) VALUES
...
-- 机柜
INSERT INTO attribute_definitions (name, `key`, category, type, sort_order, description) VALUES
('机柜', 'cabinet', 'location', 'text', NULL, 3, '主机所在的机柜');

-- 环境
INSERT INTO attribute_definitions (name, `key`, category, type, options, default_value, required, sort_order, description) VALUES
...
-- SELECT
--     s.id,
--     (SELECT id FROM attribute_definitions WHERE `key` = 'room'),
--     'room',
--     c.code,
--     'location'
-- FROM servers s
