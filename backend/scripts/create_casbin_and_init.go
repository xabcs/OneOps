...
func main() {
	fmt.Println("========================================")
	fmt.Println("创建Casbin表并初始化API权限")
	fmt.Println("========================================")
	...
	CREATE TABLE IF NOT EXISTS casbin_rule (
		...
		INDEX idx_ptype (p_type)
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Casbin策略表';
	`

	if err := db.Exec(createCasbinTable).Error; err != nil {
		log.Printf("创建Casbin表失败: %v", err)
	} else {
		fmt.Println("✓ Casbin表创建完成")
	}

	// 检查表是否存在
	var count int64
	db.Exec("SELECT COUNT(*) FROM casbin_rule").Count(&count)
	fmt.Printf("  当前casbin_rule表记录数: %d\n", count)
	...
	fmt.Println("\n========================================")
	fmt.Println("Casbin表创建和API权限初始化完成！")
	fmt.Println("========================================")
}
	...
	for _, api := range allAPIs {
		for _, method := range api.Methods {
			if err := permService.AssignAPIPermission("admin", api.Path, method); err != nil {
				log.Printf("添加admin权限失败: %s %s - %v", api.Path, method, err)
			}
		}
	}
...
func setupOpsPermissions(permService *services.PermissionService) error {
	...
	for _, perm := range opsPermissions {
		if err := permService.AssignAPIPermission("ops", perm.Path, perm.Method); err != nil {
			log.Printf("添加ops权限失败: %s %s - %v", perm.Path, perm.Method, err)
		}
	}
...
func setupViewerPermissions(permService *services.PermissionService) error {
	...
	for _, perm := range viewerPermissions {
		if err := permService.AssignAPIPermission("viewer", perm.Path, perm.Method); err != nil {
			log.Printf("添加viewer权限失败: %s %s - %v", perm.Path, perm.Method, err)
		}
	}
