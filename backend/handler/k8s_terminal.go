...
func NewK8sTerminalHandler(cnt *container.ServiceContainer) *K8sTerminalHandler {
...
func (h *K8sTerminalHandler) HandleWebSocket(c *gin.Context) {
	...
	// 检查 Pod 是否存在
	clientset, _, err := h.container.K8sClientPool().GetClient(uint(clusterID))
	if err != nil {
	...
	if err := h.container.GetDB().Create(session).Error; err != nil {
...
func (h *K8sTerminalHandler) createExecutor(session *K8sSession, size *K8sStreamSize) (remotecommand.Executor, error) {
	clientset, config, err := h.container.K8sClientPool().GetClient(session.clusterID)
	if err != nil {
...
// handleSession 处理会话
func (h *K8sTerminalHandler) handleSession(session *K8sSession, dbSession *models.K8sSession, size *K8sStreamSize) {
	// 创建 Streamer 接口实现
...
// closeSession 关闭会话
func (h *K8sTerminalHandler) closeSession(session *models.K8sSession, reason string) {
	now := time.Now()
	...
	if err := h.container.GetDB().Save(session).Error; err != nil {
