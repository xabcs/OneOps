...
func (s *K8sClusterService) GetClusters(userID uint, page, pageSize int, filter *models.K8sClusterFilter) ([]models.K8sCluster, int64, error) {
	...
	// 权限过滤：只返回用户有权限的集群
	var filteredClusters []models.K8sCluster
...
func (s *K8sClusterService) GetClusterByID(clusterID uint, userID uint) (*models.K8sCluster, error) {
...
// CreateCluster 创建新集群
func (s *K8sClusterService) CreateCluster(cluster *models.K8sCluster, kubeconfigPlaintext string, operatorID uint) error {
	// 加密 kubeconfig
...
func (s *K8sClusterService) DeleteCluster(clusterID uint, confirmName string, operatorID uint) error {
	...
	// 删除角色绑定
	if err := tx.Where("cluster_id = ?", clusterID).Delete(&models.ClusterRoleBinding{}).Error; err != nil {
		tx.Rollback()
		return fmt.Errorf("删除集群角色绑定失败: %w", err)
	}

	// 删除会话记录
	if err := tx.Where("cluster_id = ?", clusterID).Delete(&models.K8sSession{}).Error; err != nil {
		tx.Rollback()
...
func (s *K8sClusterService) AssignClusterRole(userID uint, clusterID uint, roleID uint, operatorID uint) error {
	...
	// 检查是否已存在绑定
	var existingBinding models.ClusterRoleBinding
	...
	if err == nil {
		...
		// 不存在，创建新绑定
		binding := models.ClusterRoleBinding{
			UserID:    userID,
			ClusterID: clusterID,
			RoleID:    roleID,
		}
...
func (s *K8sClusterService) RevokeClusterRole(userID uint, clusterID uint, operatorID uint) error {
	result := s.db.Where("user_id = ? AND cluster_id = ?", userID, clusterID).Delete(&models.ClusterRoleBinding{})
	if result.Error != nil {
...
func (s *K8sClusterService) CheckUserClusterAccess(userID uint, clusterID uint) (bool, error) {
	...
	// 检查是否存在角色绑定
	var count int64
	err := s.db.Model(&models.ClusterRoleBinding{}).
