...
func (g *GenericAdapter) FetchUsers(baseURL string, authConfig map[string]interface{}) ([]models.ApplicationUser, error) {
	...
	// 解析响应
	var result struct {
		Data []models.ApplicationUser `json:"data"`
	}
	body, _ := io.ReadAll(resp.Body)
...
func (g *GenericAdapter) FetchRoles(baseURL string, authConfig map[string]interface{}) ([]models.ApplicationRole, error) {
	// 获取 endpoints 配置（支持多种类型）
	...
	// 解析响应
	var result struct {
		Data []models.ApplicationRole `json:"data"`
	}
	body, _ := io.ReadAll(resp.Body)
	if err := json.Unmarshal(body, &result); err != nil {
...
func (g *GenericAdapter) FetchGroups(baseURL string, authConfig map[string]interface{}) ([]models.ApplicationGroup, error) {
