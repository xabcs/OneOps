...
func (g *GenericAdapter) FetchUsers(baseURL string, authConfig map[string]interface{}) ([]ApplicationUser, error) {
...
func (g *GenericAdapter) FetchRoles(baseURL string, authConfig map[string]interface{}) ([]ApplicationRole, error) {
	// 获取 endpoints 配置（支持多种类型）
	...
	// 解析响应
	var result struct {
		Data []ApplicationRole `json:"data"`
	}
	body, _ := io.ReadAll(resp.Body)
	if err := json.Unmarshal(body, &result); err != nil {
...
func (g *GenericAdapter) FetchGroups(baseURL string, authConfig map[string]interface{}) ([]ApplicationGroup, error) {
