...
func (g *GitLabAdapter) FetchUsers(baseURL string, authConfig map[string]interface{}) ([]ApplicationUser, error) {
	// 获取认证信息
	...
	// 转换为 ApplicationUser
	users := make([]ApplicationUser, 0, len(usersData))
...
func (g *GitLabAdapter) FetchRoles(baseURL string, authConfig map[string]interface{}) ([]ApplicationRole, error) {
	// GitLab 的访问级别是预定义的
...
func (g *GitLabAdapter) FetchGroups(baseURL string, authConfig map[string]interface{}) ([]ApplicationGroup, error) {
	// 获取认证信息
	token, ok := authConfig["token"].(string)
	if !ok || token == "" {
		return nil, fmt.Errorf("GitLab 需要认证 token")
	...
	result := make([]ApplicationGroup, 0, len(groups))
