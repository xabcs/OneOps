...
func (j *JumpserverAdapter) FetchUsers(baseURL string, authConfig map[string]interface{}) ([]ApplicationUser, error) {
	// 获取端点配置（支持多种类型）
	...
	// 转换为 ApplicationUser
	users := make([]ApplicationUser, 0, len(apiUsers))
	for _, user := range apiUsers {
		...
		users = append(users, ApplicationUser{
...
func (j *JumpserverAdapter) FetchGroups(baseURL string, authConfig map[string]interface{}) ([]ApplicationGroup, error) {
	// 获取端点配置（支持多种类型）
	...
	groups := make([]ApplicationGroup, 0, len(apiGroups))
...
func (j *JumpserverAdapter) FetchRoles(baseURL string, authConfig map[string]interface{}) ([]ApplicationRole, error) {
...
func (j *JumpserverAdapter) FetchAuthorizationRules(baseURL string, authConfig map[string]interface{}) ([]ApplicationAuthorizationRule, error) {
	...
	return []ApplicationAuthorizationRule{}, nil
}

// parseAuthorizationRules 解析授权规则响应数据
func (j *JumpserverAdapter) parseAuthorizationRules(body []byte, url string) ([]ApplicationAuthorizationRule, error) {
	...
	rules := make([]ApplicationAuthorizationRule, 0)
	for i, apiRule := range apiRules {
		...
		rules = append(rules, ApplicationAuthorizationRule{
			RuleID:      apiRule.ID,
