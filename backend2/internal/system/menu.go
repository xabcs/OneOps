...
func (ctrl *MenuController) buildMenuTree(menus []Menu, parentID uint) []*Menu {
	var result []*Menu
...
func (ctrl *MenuController) UpdateMenu(c *gin.Context) {
	...
	if err := db.Model(&Menu{}).Where("id = ?", id).Updates(updates).Error; err != nil {
...
func (ctrl *MenuController) DeleteMenu(c *gin.Context) {
	...
	// 检查是否有子菜单
	var count int64
	db.Model(&Menu{}).Where("parent_id = ?", id).Count(&count)
	if count > 0 {
