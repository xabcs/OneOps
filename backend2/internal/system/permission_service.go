...
// getUserRoles 获取用户角色
func (s *PermissionService) getUserRoles(userID uint) ([]Role, error) {
	var user User
	if err := s.db.First(&user, userID).Error; err != nil {
		return nil, err
	}
...
func (s *PermissionService) isSuperAdmin(roles []Role) bool {
	for _, role := range roles {
...
func (s *PermissionService) GetRolePermissions(roleID uint) ([]Permission, error) {
	var permissions []Permission
...
func (s *PermissionService) RevokePermissionFromRole(roleID uint, permissionID uint) error {
	if err := s.db.Where("role_id = ? AND permission_id = ?", roleID, permissionID).Delete(&RolePermission{}).Error; err != nil {
		return err
	}
...
func (s *PermissionService) GetPermissionList(page, size int, query map[string]interface{}) ([]Permission, int64, error) {
	var permissions []Permission
	var total int64
...
func (s *PermissionService) CreatePermission(permission *Permission) error {
...
func (s *PermissionService) UpdatePermission(permission *Permission) error {
...
func (s *PermissionService) DeletePermission(id uint) error {
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		// 删除用户权限关联
		if err := tx.Where("permission_id = ?", id).Delete(&UserPermission{}).Error; err != nil {
			return err
		}
...
func (s *PermissionService) GetPermissionTree() ([]Permission, error) {
	var permissions []Permission
...
func (s *PermissionService) buildPermissionTree(permissions []Permission, parentID *uint) []Permission {
	var tree []Permission
...
func (s *PermissionService) syncRoleToCasbin(role *Role) error {
	// 删除角色的所有旧策略
