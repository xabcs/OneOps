...
// menuSyncFunc 菜单同步回调函数（由 cmd/api 在启动时注入，避免循环依赖）
var menuSyncFunc func() error

// SetMenuSyncFunc 设置菜单同步回调
func SetMenuSyncFunc(f func() error) {
	menuSyncFunc = f
}
...
func (c *RouteController) convertMenusToRoutes(menus []*Menu, isSuper bool) []map[string]interface{} {
...
func (c *RouteController) buildRouteFromMenu(menu *Menu, hasChildren bool, isSuper bool) map[string]interface{} {
	// 路由名称（从路径生成）
