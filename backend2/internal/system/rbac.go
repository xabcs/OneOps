...
func (s *RBACService) BuildMenuTreeAndPermissions(userID uint) ([]*Menu, []string, []*Role, error) {
	startTime := time.Now()
...
func (s *RBACService) buildMenuTree(allMenus []*Menu, menuIDs map[uint]bool, parentID uint) []*Menu {
	var result []*Menu
...
func (s *RBACService) extractPermissions(menuTree []*Menu) []string {
	permissions := make([]string, 0)
