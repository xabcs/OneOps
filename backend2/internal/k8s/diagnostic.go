...
func NewDiagnosticController(clusterSvc *K8sClusterService, clientPool *K8sClientPool, db *gorm.DB) *DiagnosticController {
...
func (ctrl *DiagnosticController) GetJavaPods(c *gin.Context) {
	...
	// 1. 获取K8s客户端
	client, _, err := ctrl.clientPool.GetClient(uint(clusterID))
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code":    500,
...
func (ctrl *DiagnosticController) ExecuteDiagnostic(c *gin.Context) {
	...
	if cluster, _ := ctrl.clusterService.GetClusterByID(uint(clusterID), userID.(uint)); cluster != nil {
	...
	// 保存到数据库
	if dbErr := ctrl.db.Create(&history).Error; dbErr != nil {
...
func (ctrl *DiagnosticController) GetDiagnosticHistory(c *gin.Context) {
	...
	var histories []DiagnosticHistory
	var total int64

	query := ctrl.db.Model(&DiagnosticHistory{})
