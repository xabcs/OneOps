...
func NewK8sClusterController(svc *K8sClusterService) *K8sClusterController {
	return &K8sClusterController{clusterService: svc}
}
...
func (ctrl *K8sClusterController) GetClusters(c *gin.Context) {
	...
	clusters, total, err := ctrl.clusterService.GetClusters(uid, page, pageSize, filter)
	if err != nil {
...
func (ctrl *K8sClusterController) GetClusterByID(c *gin.Context) {
	...
	// 获取集群详情
	cluster, err := ctrl.clusterService.GetClusterByID(uint(clusterID), userID.(uint))
	if err != nil {
...
func (ctrl *K8sClusterController) CreateCluster(c *gin.Context) {
	...
	// 创建集群
	if err := ctrl.clusterService.CreateCluster(cluster, req.Kubeconfig, userID.(uint)); err != nil {
...
func (ctrl *K8sClusterController) UpdateCluster(c *gin.Context) {
	...
	// 更新集群
	if err := ctrl.clusterService.UpdateCluster(uint(clusterID), updates, userID.(uint)); err != nil {
...
func (ctrl *K8sClusterController) DeleteCluster(c *gin.Context) {
	...
	// 删除集群（带二次确认）
	if err := ctrl.clusterService.DeleteCluster(uint(clusterID), req.ConfirmName, userID.(uint)); err != nil {
...
func (ctrl *K8sClusterController) TestConnection(c *gin.Context) {
	...
	// 测试连接
	if err := ctrl.clusterService.TestConnection(uint(clusterID)); err != nil {
...
func (ctrl *K8sClusterController) GetClusterNodes(c *gin.Context) {
	...
	// 获取节点列表
	nodes, err := ctrl.clusterService.GetClusterNodes(uint(clusterID))
	if err != nil {
...
func (ctrl *K8sClusterController) GetClusterNamespaces(c *gin.Context) {
	...
	// 获取命名空间列表
	namespaces, err := ctrl.clusterService.GetClusterNamespaces(uint(clusterID))
	if err != nil {
...
func (ctrl *K8sClusterController) GetClusterUsers(c *gin.Context) {
	...
	// 获取用户列表
	users, err := ctrl.clusterService.GetClusterUsers(uint(clusterID))
	if err != nil {
