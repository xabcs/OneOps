...
func NewK8sPermissionController(clusterSvc *K8sClusterService, db *gorm.DB) *K8sPermissionController {
...
func (ctrl *K8sPermissionController) AssignClusterRole(c *gin.Context) {
	...
	// 分配角色
	if err := ctrl.clusterService.AssignClusterRole(req.UserID, uint(clusterID), req.RoleID, operatorID.(uint)); err != nil {
...
func (ctrl *K8sPermissionController) RevokeClusterRole(c *gin.Context) {
	...
	// 撤销角色
	if err := ctrl.clusterService.RevokeClusterRole(uint(userID), uint(clusterID), operatorID.(uint)); err != nil {
...
func (ctrl *K8sPermissionController) GetUserClusters(c *gin.Context) {
	...
	// 获取集群列表（已包含权限过滤）
	clusters, total, err := ctrl.clusterService.GetClusters(userID.(uint), page, pageSize, nil)
	if err != nil {
...
func (ctrl *K8sPermissionController) BatchAssignClusterRoles(c *gin.Context) {
	...
	for _, userID := range req.UserIDs {
		if err := ctrl.clusterService.AssignClusterRole(userID, req.ClusterID, req.RoleID, operatorID.(uint)); err != nil {
