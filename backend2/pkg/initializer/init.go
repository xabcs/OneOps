...
func (s *InitService) initDiagnosticPermissions() error {
	...
	permissions := []k8s.DiagnosticPermission{
		{
			Name:        "执行K8s诊断",
	...
	for _, permission := range permissions {
		var existingPerm k8s.DiagnosticPermission
...
func (s *InitService) initDiagnosticConfig() error {
	...
	for _, config := range configs {
		var existingConfig k8s.DiagnosticConfig
...
func (s *InitService) syncMenus() error {
	...
	for _, mapping := range resourceMappings {
		result := db.Model(&system.Menu{}).Where("path = ?", mapping.path).Update("resource", mapping.resource)
		if result.Error != nil {
	...
	deletedK8sAuditResult := db.Where("path = ?", "/k8s/audit/sessions").Delete(&system.Menu{})
	...
	deletedResult := db.Where("path = ? OR name IN ?", "/cmdb/groups", []string{"主机分组", "cmdb_groups"}).Delete(&system.Menu{})
	...
	deletedTerminalResult := db.Where("path IN ?", []string{"/terminal", "/terminal/workbench"}).Delete(&system.Menu{})
...
func (s *InitService) initAttributes() error {
	...
	attributes := []system.AttributeDefinition{
		{
			Name:        "业务系统",
			Key:         "business_system",
			Category:    "system",
...
func (s *InitService) syncAttributes() error {
	...
	for _, builtinAttr := range builtinAttributes {
		var existingAttr system.AttributeDefinition
		// 使用 GORM 的方式查询，避免 SQL 保留字问题
		err := db.Where(&system.AttributeDefinition{Key: builtinAttr.key}).First(&existingAttr).Error
		...
		if err == nil {
			...
			// 属性不存在，添加新属性
			newAttr := system.AttributeDefinition{
				Name:         builtinAttr.name,
				Key:          builtinAttr.key,
				Category:     builtinAttr.category,
				Type:         builtinAttr.attrType,
...
func (s *InitService) initAgentVersions() error {
	...
	defaultVersion := cmdb.AgentVersion{
		Version:       "1.0.0",
...
func (s *InitService) assignDefaultPermissions() error {
	...
	for roleCode, permissionCodes := range rolePermissions {
		...
		for _, perm := range permissions {
			var rolePerm system.RolePermission
