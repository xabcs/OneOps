...
OneOps 采用 **混合采集架构**，通过 Agent 实现服务器监控数据的采集和推送。

### 数据采集方式

| 方式 | 触发频率 | 数据方向 | 用途 |
|------|---------|---------|------|
| **Agent心跳上报** | Agent控制（定期） | Agent → Server | 状态保活、版本更新 |
| **定时HTTP拉取** | 5分钟 | Server → Agent | 数据库指标更新 |
| **手动HTTP拉取** | 用户触发 | Server → Agent | 即时刷新 |
| **WebSocket推送** | 30秒 | Server → 前端 | 实时监控概览 |
| **API查询** | 用户操作 | 前端 → Server | 获取历史数据、详情 |
...
└─────────────────────────────────────────────────────────────────┘
   ...
   └───────────┘       └───────────┘       └───────────┘
         │                    │                    │
         │ POST /agent/      │ GET /metrics      │ GET /metrics
         │ heartbeat         │                   │
         │                    │                    │
         ▼                    ▼                    ▼
   ┌─────────────────────────────────────────────────────────┐
...
| 端口 | 路径 | 说明 |
|------|------|------|
| 9100 | `GET /metrics` | 基础指标（CPU、内存、磁盘、负载） |
| 9100 | `GET /extended-metrics` | 扩展指标（进程、网络、服务、系统信息） |
...
| 消息类型 | 说明 | 数据内容 |
|---------|------|---------|
| `overview_update` | 监控概览更新 | 所有主机列表、在线/离线统计 |
| `metrics_update` | 单个主机指标更新 | CPU、内存、磁盘、进程数等 |
| `alert` | 告警事件 | 告警级别、消息、时间戳 |
