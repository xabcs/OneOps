  ...
  class y {
    ...
    _detect() {
      this._detected ||
        ...
        (this._isNode = typeof module < 'u' && Boolean(module.exports)),
    ...
    static _isWindows() {
      return typeof navigator < 'u' && navigator.userAgent && navigator.userAgent.includes('Windows')
        ? !0
        : typeof process < 'u'
...
(function (u) {
  ...
  class m {
    ...
    static mergeConfigurationOptions(r = null, c = null) {
      const a = u.Utilities.recursiveClone(c || {});
      return (
  ...
  class p {
    constructor(r, c) {
      if (
        ...
        const a = this.options.nodeRequire.main.filename;
        const t = Math.max(a.lastIndexOf('/'), a.lastIndexOf('\\'));
    ...
    moduleIdToPaths(r) {
      ...
      } else (!u.Utilities.endsWith(c, '.js') && !u.Utilities.containsQueryString(c) && (c += '.js'), (a = [c]));
...
(function (u) {
  class y {
    ...
    load(e, i, s, n) {
      ...
      const l = { callback: s, errorback: n };
  ...
  class m {
    ...
    load(e, i, s, n) {
      if (/^node\|/.test(i)) {
        ...
        const d = c(e.getRecorder(), l.nodeRequire || u.global.nodeRequire);
        const o = i.split('|');
        let _ = null;
        try {
          _ = d(o[1]);
  ...
  class r {
    ...
    _initNodeRequire(e, i) {
      ...
      l.prototype._compile = function (o, _) {
        ...
        const g = i.getRecorder();
        const v = n._getCachedDataPath(s, _);
        const E = { filename: _ };
        let I;
        try {
        ...
        const P = C.runInThisContext(E);
        const w = n._path.dirname(_);
        const R = d(this);
        const U = [this.exports, R, this, _, w, process, _commonjsGlobal, Buffer];
        const b = P.apply(this.exports, U);
    ...
    load(e, i, s, n) {
      ...
      if (/^node\|/.test(i)) {
        ...
        const g = this._getElectronRendererScriptPathOrUri(f);
        const v = Boolean(l.nodeCachedData);
        const E = v ? this._getCachedDataPath(l.nodeCachedData, i) : void 0;
        this._readSourceAndCachedData(f, E, _, (I, C, P, w) => {
          ...
          const b = this._createAndEvalScript(e, R, U, s, n);
    ...
    _createAndEvalScript(e, i, s, n, l) {
      ...
      const _ = o.runInThisContext(s);
      const f = e.getGlobalAMDDefineFunc();
      let g = !1;
      const v = function () {
    ...
    _getCachedDataPath(e, i) {
      const s = this._crypto
        ...
        .update(process.arch, '')
        .digest('hex');
      const n = this._path.basename(i).replace(/\.js$/, '');
    ...
    _createAndWriteCachedData(e, i, s, n) {
      const l = Math.ceil(n.getConfig().getOptionsLiteral().nodeCachedData.writeDelay * (1 + Math.random()));
      let d = -1;
      let o = 0;
      let _;
      const f = () => {
...
(function (u) {
    ...
    complete(t, e, i, s) {
      ...
      if (this._callback)
        if (typeof this._callback === 'function') {
          ...
          const l = m._invokeFactory(e, this.strId, this._callback, i);
          ((n = l.producedError),
  ...
  class c {
    ...
    static _findRelevantLocationInStack(t, e) {
      const i = l => l.replace(/\\/g, '/');
      const s = i(t);
      const n = e.split(/\n/);
      for (let l = 0; l < n.length; l++) {
        const d = n[l].match(/(.*):(\d+):(\d+)\)?$/);
        if (d) {
          let o = d[1];
          const _ = d[2];
          const f = d[3];
          const g = Math.max(o.lastIndexOf(' ') + 1, o.lastIndexOf('(') + 1);
          if (((o = o.substr(g)), (o = i(o)), o === s)) {
            const v = { line: Number.parseInt(_, 10), col: Number.parseInt(f, 10) };
    ...
    getBuildInfo() {
      ...
      for (let i = 0, s = this._modules2.length; i < s; i++) {
        const n = this._modules2[i];
        if (!n) continue;
        const l = this._buildInfoPath[n.id] || null;
        const d = this._buildInfoDefineStack[n.id] || null;
        const o = this._buildInfoDependencies[n.id];
        t[e++] = {
          id: n.strId,
          path: l,
    ...
    defineModule(t, e, i, s, n, l = new y(t)) {
      const d = this._moduleIdProvider.getModuleId(t);
      if (this._modules2[d]) {
        this._config.isDuplicateMessageIgnoredFor(t) || console.warn(`Duplicate definition of module '${t}'`);
        return;
      }
      const o = new m(d, t, this._normalizeDependencies(e, l), i, s, l);
      ((this._modules2[d] = o),
    ...
    _normalizeDependency(t, e) {
      ...
      const i = t.indexOf('!');
      if (i >= 0) {
        const s = e.resolveModule(t.substr(0, i));
        const n = e.resolveModule(t.substr(i + 1));
        const l = this._moduleIdProvider.getModuleId(`${s}!${n}`);
        const d = this._moduleIdProvider.getModuleId(s);
        return new r(l, d, n);
      }
    ...
    synchronousRequire(t, e = new y(t)) {
      const i = this._normalizeDependency(t, e);
      const s = this._modules2[i.id];
      if (!s)
        throw new Error(
    ...
    configure(t, e) {
      const i = this._config.shouldRecordStats();
    ...
    _onLoad(t) {
      if (this._currentAnonymousDefineCall !== null) {
        const e = this._currentAnonymousDefineCall;
        ((this._currentAnonymousDefineCall = null),
          this.defineModule(this._moduleIdProvider.getStrModuleId(t), e.dependencies, e.callback, null, e.stack));
      }
    }
    _createLoadError(t, e) {
      const i = this._moduleIdProvider.getStrModuleId(t);
      const s = (this._inverseDependencies2[t] || []).map(l => this._moduleIdProvider.getStrModuleId(l));
    ...
    _onLoadError(t, e) {
      ...
      for (l.push(t), s[t] = !0; l.length > 0; ) {
        const d = l.shift();
        const o = this._modules2[d];
        o && (n = o.onDependencyError(i) || n);
        const _ = this._inverseDependencies2[d];
        if (_)
    ...
    _findCyclePath(t, e, i) {
      ...
      const s = this._modules2[t];
      if (!s) return null;
      const n = s.dependencies;
      if (n)
        for (let l = 0, d = n.length; l < d; l++) {
          const o = this._findCyclePath(n[l].id, e, i + 1);
          if (o !== null) return (o.push(t), o);
        }
      return null;
    }
    _createRequire(t) {
      const e = (i, s, n) => this._relativeRequire(t, i, s, n);
      return (
    ...
    _loadModule(t) {
      ...
      const e = this._moduleIdProvider.getStrModuleId(t);
      const i = this._config.moduleIdToPaths(e);
      const s = /^@[^\/]+\/[^\/]+$/;
      this._env.isNode && (!e.includes('/') || s.test(e)) && i.push(`node|${e}`);
      let n = -1;
      const l = d => {
        if ((n++, n >= i.length)) this._onLoadError(t, d);
        else {
          const o = i[n];
          const _ = this.getRecorder();
            ...
            ((this._hasDependencyCycle = !0),
              console.warn(
                `There is a dependency cycle between '${this._moduleIdProvider.getStrModuleId(
                  n.id
                )}' and '${this._moduleIdProvider.getStrModuleId(t.id)}'. The cyclic path follows:`
              ));
            const d = this._findCyclePath(n.id, t.id, 0) || [];
            (d.reverse(),
              d.push(n.id),
              console.warn(
...
`)
          ...
          if (
            ...
            const d = this._modules2[n.pluginId];
            if (d && d.isComplete()) {
