...
const selectedIngresses = ref<any[]>([]);
...
// Service YAML 编辑
async function handleServiceEdit(row: any) {
...
// Ingress YAML 编辑
async function handleIngressEdit(row: any) {
  if (!selectedCluster.value) {
    message.warning('请先选择集群');
    return;
  }
...
// 删除 Service
async function handleServiceDelete(row: any) {
  try {
...
// 删除 Ingress
async function handleIngressDelete(row: any) {
  try {
