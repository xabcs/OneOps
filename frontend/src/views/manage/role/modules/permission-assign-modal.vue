...
const title = computed(() => props.viewOnly
...
async function getAllPermissions() {
  ...
  if (!error && data) {
    ...
    permissions.forEach((perm: Api.SystemManage.Permission) => {
      ...
      if (perm.level === 1) {
        ...
        if (!moduleMap.has(perm.module)) {
          ...
          console.log('✅ 创建模块节点:', { id: moduleNode.id, label: moduleNode.label });
        }
        ...
        console.log('✅ 创建页面节点:', { id: pageNode.id, label: pageNode.label });
        ...
        if (pageNode) {
          ...
          console.log('➕ 添加按钮节点:', { id: buttonNode.id, label: buttonNode.label, parent: pageNode.label });
        } else {
          // 父页面不存在，尝试创建（兜底逻辑）
          ...
          console.log('✅ 自动创建页面并添加按钮:', { buttonId: buttonNode.id, label: buttonNode.label });
        }
      }
    });
    ...
    console.log('🌳 权限树构建完成:', {
      模块数: tree.length,
      总节点数: countAllNodes(tree),
      虚拟节点数: countVirtualNodes(tree),
      真实节点数: countRealNodes(tree)
    });
    ...
    collectIds(tree);
    console.log('📋 权限树中所有真实ID (已排序):', allRealIds.sort((a, b) => a - b));
  } else {
...
async function getRolePermissions() {
  ...
  console.log('🔍 开始获取角色权限, roleId:', props.roleId);
  ...
  if (!error && data) {
    ...
    if (treeRef.value) {
      try {
        ...
        console.log('🔑 提取的角色权限IDs:', rolePermissionIds);
        ...
        const buttonLevelPermissions = rolePermissionIds.filter(id => {
          ...
          if (!isButtonLevel && id > 0) {
            console.log('🚫 过滤非按钮权限:', id, '级别:', nodeLevel);
          }
          return isButtonLevel;
        });

        console.log('🎯 最终设置的按钮权限IDs:', {
          原始数量: rolePermissionIds.length,
          过滤后数量: buttonLevelPermissions.length,
          过滤掉: rolePermissionIds.filter(id => !buttonLevelPermissions.includes(id))
        });

        // 过滤出有效的权限ID（正整数）
        const validPermissionIds = buttonLevelPermissions.filter((id: any) => {
        ...
        if (validPermissionIds.length > 0) {
          ...
          console.log('✅ 权限选中状态已设置:', {
            设置的IDs: validPermissionIds,
            完全选中: currentCheckedKeys.length,
            半选中: currentHalfCheckedKeys.length,
            总选中: currentCheckedKeys.length + currentHalfCheckedKeys.length
          });
...
async function handleSubmit() {
  ...
  console.log('🔍 过滤后的真实权限IDs:', realPermissionIds);
...
function getResourceLabel(resource: string, permissions: Api.SystemManage.Permission[]): string {
  ...
  // 移除常见的操作前缀
  const allPrefixes = ['查看', '列表', '查询', '创建', '新增', '添加', '编辑', '修改', '更新', '删除', '移除', '重置', '分配'];

  for (const prefix of allPrefixes) {
    if (resourceName.startsWith(prefix)) {
      resourceName = resourceName.substring(prefix.length);
      console.log(`✂️ 移除前缀 "${prefix}":`, firstPerm.name, '→', resourceName);
      break;
    }
  }
  ...
  >
    <div v-loading="loading" class="permission-assign-container">
      ...
      <div class="permission-stats">
        <ElText type="info">
          已选择 <ElText type="primary">{{ getSelectedCount() }}</ElText> 个权限
        </ElText>
      </div>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <ElButton v-if="!props.viewOnly" type="primary" :loading="loading" @click="handleSubmit">
          确定
        </ElButton>
      </div>
    </template>
