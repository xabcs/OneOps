<template>
  <div class="page-container permission-management-page">
    ...
    <!-- 内容卡片 -->
    <div
      class="content-card permission-content-card"
      :style="{
  ...
  columns: () => [
    ...
    { prop: 'action', label: $t('page.manage.permission.action'), minWidth: 100 },
    {
      ...
      formatter: row => {
        ...
        return <ElTag size="small" type={level.type}>{level.text}</ElTag>;
      }
    },
    {
      prop: 'status',
    ...
    },
    {
      ...
      formatter: row => (
        <div class="flex-center gap-8px">
          ...
          <ElButton type="info" plain size="small" onClick={() => assignRoles(row.id)}>
            分配角色
          </ElButton>
...
// 角色分配相关
const { bool: assignModalVisible, setTrue: openAssignModal } = useBoolean();
const currentPermissionId = ref<number>(-1);
const currentPermissionData = ref<Api.SystemManage.Permission | null>(null);

function assignRoles(id: number) {
  const permission = data.value.find(p => p.id === id);
  if (permission) {
    currentPermissionId.value = id;
    currentPermissionData.value = permission;
    openAssignModal();
  }
}

async function handleAssignSubmitted() {
  await getData();
}
