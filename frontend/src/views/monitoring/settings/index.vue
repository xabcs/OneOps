...
const currentRule = ref<AlertRuleForm>({
  name: '',
  level: 'high',
  metric: 'cpu_usage',
  condition: '>',
...
// 切换告警规则状态
async function handleToggleRuleStatus(rule: AlertRule) {
  try {
...
const notificationChannelsData = ref<NotificationChannel[]>([]);
const notificationLoading = ref(false);
const showChannelDialog = ref(false);
const channelDialogMode = ref<'create' | 'edit'>('create');
const currentChannel = ref<NotificationChannelForm>({
  channelType: 'email',
  channelName: '',
  config: {} as any,
  enabled: true
...
async function handleSaveChannel() {
  ...
  if (currentChannel.value.channelType === 'email') {
    const config = currentChannel.value.config as any;
...
// 获取渠道类型图标
function getChannelIcon(type: string): any {
                ...
                <template #default="{ row }">
                  <div v-if="row.channelType === 'email'" class="text-sm">
                    <div class="text-gray-600">
                      SMTP: {{ (row.config as any).smtpHost }}:{{ (row.config as any).smtpPort }}
                    </div>
                    <div class="text-gray-600">发件人: {{ (row.config as any).from }}</div>
                  </div>
                  <div v-else-if="row.channelType === 'wechat'" class="text-sm">
                    <div class="truncate text-gray-600" :title="(row.config as any).webhookUrl">
                      Webhook: {{ (row.config as any).webhookUrl }}
                    </div>
                  </div>
      ...
      <ElForm :model="currentChannel" label-width="120px">
        ...
        <template v-if="currentChannel.channelType === 'email'">
          ...
          <ElFormItem label="SMTP 服务器" required>
            <ElInput v-model="(currentChannel.config as any).smtpHost" placeholder="smtp.example.com" />
            <span class="ml-2 text-gray-500">服务器地址</span>
          </ElFormItem>

          <ElFormItem label="SMTP 端口" required>
            <ElInputNumber
              v-model="(currentChannel.config as any).smtpPort"
              :min="1"
              :max="65535"
          ...
          <ElFormItem label="用户名">
            <ElInput v-model="(currentChannel.config as any).username" placeholder="认证用户名" />
          </ElFormItem>

          <ElFormItem label="密码">
            <ElInput
              v-model="(currentChannel.config as any).password"
              type="password"
              placeholder="认证密码"
              show-password
            />
          ...
          <ElFormItem label="发件人邮箱" required>
            <ElInput v-model="(currentChannel.config as any).from" placeholder="noreply@example.com" />
          </ElFormItem>

          <ElFormItem label="发件人名称">
            <ElInput v-model="(currentChannel.config as any).fromName" placeholder="OneOps 监控" />
          </ElFormItem>

          <ElFormItem label="启用 TLS">
            <ElSwitch v-model="(currentChannel.config as any).useTLS" />
        ...
        <template v-if="currentChannel.channelType === 'wechat'">
          ...
          <ElFormItem label="Webhook URL" required>
            <ElInput
              v-model="(currentChannel.config as any).webhookUrl"
              type="textarea"
              :rows="3"
